...
from tests.test_executor_gate import VALID_PLAN, VALID_POLICY, _gate_inputs
...
def test_executor_contract_defines_future_confirmation_and_refusals(tmp_path):
    ...
    assert gate_data["title"] == "Autonomous Forge validation executor contract preview"
    assert gate_data["mode"] == "read-only"
    assert gate_data["future_confirmation_flag"] == "--confirm-executor-dry-run"
    assert gate_data["executor_dry_run_allowed_now"] is False
    assert gate_data["validation_execution"] == "not run"
    assert "future caller omits --confirm-executor-dry-run" in gate_data["refusal_cases"]
    assert gate_data["result_capture_shape"]["write_command"] == "forge validation-result-write --confirm-write"
    assert gate_data["timeout_policy"]["default_seconds"] == 300
...
def test_executor_contract_formats_text(tmp_path):
    (tmp_path / "AUTONOMOUS_STATE.md").write_text("# State\n", encoding="utf-8")
    _write_record(tmp_path, "passed.json", payload=_payload("AUTO-044", "Passed validation", "passed"))
