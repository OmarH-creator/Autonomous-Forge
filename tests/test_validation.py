...
def test_build_validation_plan_data_uses_proposal_data(tmp_path):
    ...
    assert validation["implementation_steps"] == [
        "Inspect roadmap task AUTO-023, repository policy, state file, and documented command contracts before editing.",
        "Limit the implementation scope to: Stay read-only and stdout-only.",
        "Update the expected file areas only when needed: `src/autonomous_forge/validation.py`, `src/autonomous_forge/cli.py`, tests, `.env`, docs.",
        "Satisfy the documented acceptance criteria: Path checks are printed without reading secrets or running commands.",
    ...
    assert any(item["source"] == "policy" and item["risk"] == "Adding network access." for item in validation["risk_register"])
    ...
    assert validation["path_checks"] == [
        {"area": "src/autonomous_forge/validation.py", "path_status": "present", "policy_status": "allowed"},
        {"area": "src/autonomous_forge/cli.py", "path_status": "missing", "policy_status": "allowed"},
...
def test_validation_path_checks_do_not_follow_symlink_outside_root(tmp_path):
    ...
    plan = VALID_PLAN.replace("`src/autonomous_forge/validation.py`, `src/autonomous_forge/cli.py`, tests, `.env`, docs.", "`src`, tests.")
    plan_data = build_repository_plan_data(plan, VALID_POLICY, state_path=state, root=tmp_path)
    proposal_data = build_change_proposal_data(plan_data)

    validation = build_validation_plan_data(proposal_data, root=tmp_path)

    assert validation["path_checks"][0] == {"area": "src", "path_status": "unknown", "policy_status": "allowed"}
...
def test_build_validation_plan_formats_reviewable_output(tmp_path):
    ...
    output = build_validation_plan(VALID_PLAN, VALID_POLICY, state_path=state, root=tmp_path)
    ...
    assert "roadmap" in output and "Path checks are advisory only" in output
...
def test_build_validation_plan_supports_json_output(tmp_path):
    ...
    output = build_validation_plan(VALID_PLAN, VALID_POLICY, state_path=state, root=tmp_path, output_format="json")
    ...
    assert data["implementation_steps"][0].startswith("Inspect roadmap task AUTO-023")
...
def test_validate_plan_command_prints_validation_plan(tmp_path, capsys):
    ...
    assert main(["validate-plan", "--plan", str(plan), "--policy", str(policy), "--state", str(state), "--root", str(tmp_path)]) == 0
...
def test_validate_plan_command_prints_json(tmp_path, capsys):
    ...
    assert main(["validate-plan", "--plan", str(plan), "--policy", str(policy), "--state", str(state), "--root", str(tmp_path), "--format", "json"]) == 0
