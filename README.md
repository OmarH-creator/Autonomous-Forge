...
Latest stewardship run: **AUTO-179 — persist immutable preservation receipts**.

- **Changed:** added `forge maintenance-preservation-receipt` with preview, explicitly confirmed persistence, and verification. A receipt is a compact `maintenance-preservation-receipt/v1` hash binding to one already-complete preservation artifact; it records commit/package identity and advisory provenance without re-running archive verification.
- **Safety:** writes are confined to `.ai/preservation-receipts/*.json`, require `--confirm-write`, refuse existing outputs and symlink inputs, use a flushed same-directory temporary file plus atomic no-clobber hard-link and directory fsync, and preserve external validation as non-executor-equivalent with no preservation-gate effect.
- **Branch/PR disposition:** work stayed on `main`. Historical branches remain stale/diverged; reviewed PRs are merged/closed/obsolete or unrelated and none warranted integration.
- **Validation:** the new receipt core, CLI, and focused tests syntax-compile in the available scratch environment. Deterministic coverage includes complete-artifact gating, independent confirmation, no-clobber persistence, exact source-byte/SHA drift detection, output confinement, and primary `forge ... --help` routing. AUTO-178 status/workflow lookups still expose no check objects, so no unsupported green-matrix claim is carried forward.
- **Visual updates:** none; the receipt is a durable terminal record on the existing preservation edge, so the lifecycle diagram remains accurate.
- **Current limitations:** receipt hashes prove byte continuity, not signer identity or validation sufficiency. Receipt verification intentionally trusts the existing preservation-completeness contract rather than duplicating lower-level archive checks.
- **Next autonomous objective:** inspect AUTO-179 CI when observable; if green, expose verified receipt discovery through preservation review without making receipt existence a substitute for preservation completeness.
