...
Executor handoff persistence is available as a programmatic helper for reviewed `executor-run --format json` output. It validates the repository-local executor-output file and handoff shape, preserves failed results, and delegates writes to the guarded validation-result writer only after explicit confirmation.
