...
After all retained validation commands pass, verified change orchestration hashes the exact target bytes into commit-readiness evidence. Verified commit creation re-hashes that target immediately before staging and refuses the commit if the file changed after validation. After staging, Forge hashes the exact index bytes for the target and requires that staged SHA-256 to match the validated target digest, and it inspects the complete NUL-delimited staged path set and requires it to exactly equal the reviewed paths. After rechecking the reviewed parent `HEAD`, Forge now repeats both index checks immediately before `git commit`, recording the final staged digest and path set. If the index changed between the first review and that final boundary, commit creation is blocked. After Git creates the commit, Forge verifies the target bytes, exact changed paths, and created commit parent before the commit can be marked verified.
...
- validation-to-commit target hashing verifies working-tree bytes before staging, exact staged index bytes afterward, the complete staged path set, then repeats the target/index path checks immediately before commit creation after confirming the reviewed parent `HEAD`. These are fail-closed checks rather than a shared Git index/ref lock, so an extremely narrow concurrent index/ref mutation may still create a commit; such a commit is detected by the exact parent/target/path post-commit checks and reported as `created_unverified` rather than silently accepted;
...
Latest stewardship run: **AUTO-197 — revalidate the Git index immediately before commit creation**.

- **Changed:** verified commit creation now performs a second staged-target SHA-256 check and a second NUL-safe complete staged-path-set check after the reviewed-parent `HEAD` check and immediately before invoking `git commit`. The final observations are retained as `precommit_staged_target_sha256` and `precommit_staged_paths`.
- **Why:** AUTO-196 prevented ordinary pre-staged contamination, but the index could still change after that first index review and before commit creation. That mutation would only be detected after a commit existed. AUTO-197 blocks that intermediate drift before `git commit`.
- **Safety:** a changed staged target or changed staged path set now fails closed before commit creation. Existing validation-byte binding, explicit commit confirmation, reviewed-parent binding, Git hooks, post-commit SHA/summary/parent/path/target verification, non-force push behavior, and no remote/protection mutation guarantees remain intact.
...
- **Validation:** deterministic AUTO-197 coverage simulates both staged-byte drift and staged-path drift between the first index review and final pre-commit revalidation; both cases must block before `git commit`. Direct clone/full pytest remains unavailable because this runtime cannot resolve `github.com`; the Python 3.10/3.11/3.12 matrix is not claimed green without observable CI evidence.
- **Visual updates:** none; this strengthens an existing verified-commit boundary and does not change lifecycle topology.
- **Current limitations:** this materially narrows but does not eliminate the race because Forge does not hold a shared Git index/ref lock. A mutation after the final revalidation and before Git consumes the index can still create a commit, but the existing post-commit exact parent/target/path checks detect it as `created_unverified`.
- **Next autonomous objective:** inspect AUTO-197 CI when observable; any failure takes priority. If green, continue the integrated maintenance milestone only with another concrete cross-stage integrity defect or meaningful evidence-handoff reduction.
