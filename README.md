...
For externally supplied validation observations, new workflows can use `forge validation-result-attachment-write` to create a separate immutable JSON sidecar under `.ai/run-history/validation-attachments/`. The sidecar leaves the original run-history record byte-for-byte unchanged, binds itself to that exact source with SHA-256 and byte count, refuses overwrite/path escape/stale source bytes, and can be re-verified against later source drift. `forge run-history-read` performs a bounded non-recursive discovery pass and surfaces verified sidecars that explicitly bind to the selected source record while preserving legacy `run-history/v1` fields unchanged. `forge maintenance-replay-summary --validation-record ...` can now carry those verified sidecars into replay output as fingerprinted **external advisory provenance**; retained validation context must agree with the bundle, and the observations are explicitly marked as not equivalent to executor-produced validation proof. The historical in-place `forge validation-result-write` remains available for backward compatibility.
...
- immutable validation sidecars are visible through `run-history-read` and can be carried into maintenance replay as advisory provenance, but canonical maintenance-bundle construction/verification does not yet persist those external observations as a separate provenance class;
...
Latest stewardship run: **AUTO-171 — preserve immutable validation provenance in maintenance replay**.

- **Changed:** extended the existing `forge maintenance-replay-summary` command with optional `--validation-record` evidence. Matching immutable validation sidecars are discovered through the existing bounded run-history reader, re-verified against their source record, fingerprinted by SHA-256/byte count, and exposed in replay output with explicit external-observation provenance.
- **Safety:** external sidecars are advisory only: `executor_validation_equivalent` is always false and `replay_gate_effect` is `advisory_only`. They never satisfy or replace the bundle's executor-produced validation stage and cannot convert a blocked bundle into a replayable one. Retained validation steps must match the bundle, expected-file-change context must cover reviewed paths, and path/symlink/size checks fail closed.
...
- **Validation:** the new replay-provenance module, updated CLI, and focused AUTO-171 tests syntax-compiled in the available scratch environment. The repository's Python 3.10/3.11/3.12 workflow still installs, compiles, smoke-tests the CLI, validates the roadmap, and runs pytest. GitHub's combined-status surface currently exposes no checks for the pushed AUTO-171 SHA, so no green final-matrix claim is made without evidence.
- **Visual updates:** none; this strengthens provenance within the existing replay/durable-evidence stage and does not change the lifecycle architecture.
- **Current limitations:** external attachment provenance is now visible in replay but is intentionally not executor proof. Canonical maintenance-bundle construction/verification still does not persist these external observations as a separate provenance class. Fresh commit-trust/status/protection acquisition remains policy-gated.
- **Next autonomous objective:** inspect AUTO-171 CI when observable; if green, carry the same explicit external-observation provenance class into maintenance-bundle/history consumption without allowing it to satisfy executor-validation readiness.
