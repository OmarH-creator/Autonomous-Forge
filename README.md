...
- Latest run: AUTO-142, green-baseline recovery — canonical archive destination mapping.
- Change: fixed one shared archive-path defect in both `maintenance-archive-copy-preview` and `maintenance-archive-copy-verify`. Manifest entries that are absolute but still repository-contained are now validated and canonicalized to repository-relative paths before being mapped beneath the requested archive root. This prevents an absolute `pathlib` operand from discarding the archive-root prefix while preserving repository containment, archive-root containment, duplicate/collision checks, overwrite refusal, byte-count verification, and SHA-256 verification.
- Validation: broad inspection covered README/docs/source/tests/config/CI, roadmap/state/changelog/decisions, issue #13, recent commits, all visible branches, and PR history. The annotated matrix started this cycle at 57 failed / 597 passed. Commit `bedb09e77ab672e8716ab1fc3910a06549afd478` reduced the inspected Python 3.11 result to 51 failed / 603 passed; commit `8160132ad188033d66a6290c180bbd718fe42952` reduced it again to 23 failed / 631 passed. No archive-copy, archive-copy-verify, archive-package, or preservation-completeness tests remain in the failure summary. Install, compile, installed CLI smoke, and roadmap lint pass on Python 3.10, 3.11, and 3.12; pytest remains red on all three versions, so `main` is not claimed green.
- Visual updates: none; this defect changes path canonicalization and safety behavior inside the existing preservation workflow, and the current architecture visuals remain accurate.
- Current limitations: 23 failures remain in the inspected Python 3.11 job. They are now separate from the resolved archive path cluster: one replay-policy primary-router help assertion, stale enriched planning/validation/executor output-contract assertions, and four maintenance-review-compare failures including three fixture directory-collision cases. Direct repository cloning/test execution is unavailable in this runtime because outbound DNS to github.com is blocked. GitHub Actions also warns that the pinned checkout/setup-python revisions target deprecated Node.js 20, but that warning is not the current P0 failure cause.
- Next autonomous objective: continue issue #13 with the largest remaining stale enriched planning/validation/executor output-contract cluster, then fix the replay-policy help assertion and maintenance-review-compare fixture/output failures until Python 3.10, 3.11, and 3.12 are green.
