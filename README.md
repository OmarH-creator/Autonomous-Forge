...
After all retained validation commands pass, verified change orchestration hashes the exact target bytes into commit-readiness evidence. Verified commit creation re-hashes that target immediately before staging and refuses the commit if the file changed after validation. After staging, Forge also hashes the exact index bytes for the target and requires that staged SHA-256 to match the validated target digest before `git commit` can run. After Git creates the commit, Forge now reads the target bytes back from the created commit and requires that committed SHA-256 to match the same validated digest before the commit can be marked verified.
...
- validation-to-commit target hashing now verifies working-tree bytes before staging, exact staged index bytes afterward, and the exact target bytes stored in the created commit. It is still not a shared Git index lock, so a concurrent mutation can cause a commit to be created; such a commit is now detected and reported as `created_unverified` rather than silently accepted;
...
Latest stewardship run: **AUTO-194 — verify committed target bytes after commit creation**.

- **Changed:** verified commit creation now reads the reviewed target back from the created commit with `git show <sha>:<target>`, computes the same bounded SHA-256 used for validation/index continuity, records it as `committed_target_sha256`, and requires it to equal the validated digest before the commit can be marked verified.
- **Why:** AUTO-193 verified index bytes immediately before `git commit`, but another process could still mutate the index after that check. Existing post-commit verification checked SHA, summary, and changed paths but not the committed target bytes themselves.
- **Safety:** committed-byte drift now yields `created_unverified` with an explicit blocker. Forge intentionally does not reset or rewrite that commit automatically. The check is local, bounded to 1 MB, and adds no push, remote, network, workflow, force-push, tag-push, or branch-protection authority.
- **Branch/PR disposition:** work stayed directly on `main`; recent PRs are merged, closed, obsolete, or unrelated. No branch or PR was created or merged.
- **Validation:** deterministic regression coverage now verifies successful validation → staged index → committed target byte continuity and a simulated raced commit target becoming `created_unverified`. Direct clone/full pytest could not run because this runtime cannot resolve `github.com`; the Python 3.10/3.11/3.12 matrix is not claimed green without observable CI evidence.
- **Visual updates:** none; the lifecycle is unchanged and the existing Mermaid flow remains accurate.
- **Current limitations:** AUTO-194 detects a raced commit but does not prevent the commit object from being created and is not a shared Git index lock. Created-but-unverified commits require human inspection.
- **Next autonomous objective:** inspect AUTO-194 CI when observable; any failure takes priority. If green, evaluate whether the remaining index→commit race can be prevented with an immutable-tree/compare-and-swap ref design without bypassing repository hooks or weakening commit safeguards.
