...
- `forge verified-push-run`: verified commit → trust/status/protection readiness → separately confirmed fast-forward push → post-push verification. Workflow status can be supplied as reviewed JSON or explicitly collected live for the exact verified created commit through the existing bounded GitHub workflow-status collector.
...
- live workflow-status collection depends on a functioning authenticated GitHub CLI, verifies each returned run's head SHA against the verified commit, applies a 15-second timeout to each live `git`/`gh` subprocess, and still does not rerun workflows or prove that the configured checks are sufficient;
...
Latest stewardship run: **AUTO-204 — bound live workflow-status subprocess execution**.

- **Changed:** the existing opt-in GitHub workflow-status collector now applies a 15-second timeout to every external `git` and `gh` subprocess used by `forge commit-status-review --from-github` and `forge verified-push-run --live-status`.
- **Why:** the live-status path had a bounded workflow-run count but no execution timeout, so a hung local Git or authenticated GitHub CLI process could stall the guarded maintenance workflow indefinitely. AUTO-204 closes that concrete reliability/safety defect without adding a new command or new external-service authority.
- **Validation:** focused Python compile/smoke validation passed before publication, including successful `git`/`gh` timeout propagation and fail-closed timeout behavior. Product/test/doc head `b6d6ee0cacf3eabb3cfce0f1137589ecb9e29f8a` passed GitHub Actions run 32840874657 across Python 3.10, 3.11, and 3.12, including package installation, source compilation, installed CLI smoke tests, roadmap validation, and pytest.
- **Safety:** live collection remains explicit and metadata-only; timeouts fail closed before status review or push readiness. No workflow rerun, force-push, tag-push, remote/protection mutation, new network surface, or additional product-code command authority was introduced.
...
- **Visual updates:** none; this hardens execution bounds inside the existing live-status stage without changing lifecycle topology.
- **Current limitations:** commit trust and branch protection remain caller-supplied; live status still depends on installed/authenticated `gh`; a subprocess timeout proves only that collection did not complete in time, not why.
- **Next autonomous objective:** continue the integrated guarded-maintenance milestone with the next concrete cross-stage integrity defect or policy-permitted evidence-handoff reduction; do not add another isolated read-only command.