...
Latest stewardship run: **AUTO-163 — derive change readiness inside the full maintenance run**.

- **Changed:** generated-preview modes of `forge verified-full-maintenance-run` no longer require callers to persist a separate `change-readiness.json`. Forge now derives the existing policy-aware change-readiness contract in memory from the generated patch preview, `.forge/policy.md`, and `--status-before-commit`, then enters the same confirmed guarded apply → target-scoped live-diff verification → validation → commit → push → post-push → durable-history chain. Persisted `--change-readiness` remains supported, and legacy supplied `--preview` mode still requires it.
- **Safety:** derivation is read-only and grants no write authority. Prohibited/unknown paths, malformed patch evidence, unclear or failed pre-commit status evidence, or any reviewed-target mismatch fail closed before apply. Apply, validation, commit, push, push-evidence write, bundle write, and history-link write remain independently confirmed; post-write live-diff verification and rollback behavior are unchanged. No new network/external-service access, force-push, tag push, remote mutation, branch-protection mutation, or workflow mutation was added.
- **Validation:** the new helper, modified verified-change apply orchestration, modified full-maintenance CLI, and focused AUTO-163 tests syntax-compile in the available scratch Python environment. Full local pytest remains unavailable because this runtime cannot clone from `github.com`; the repository's Python 3.10/3.11/3.12 Actions matrix remains the authoritative full validation when its result is observable.
- **Visual updates:** none; the Mermaid lifecycle remains accurate because AUTO-163 removes an intermediate evidence-file handoff rather than adding a lifecycle stage.
- **Current limitations:** pre-commit status, commit-trust, post-commit workflow/status, and branch-protection evidence remain caller supplied. Fresh external trust/status/protection acquisition remains policy-gated because `.forge/policy.md` requires human approval before adding network/external-service access.
- **Next autonomous objective:** if AUTO-163 CI is green, continue the same end-to-end milestone by reducing the next caller-managed local evidence handoff before commit/push without weakening independent confirmation gates; external GitHub evidence acquisition remains deferred pending explicit policy approval.
