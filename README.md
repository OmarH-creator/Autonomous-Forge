...
Latest stewardship run: **AUTO-209 — expose live-status provenance on verified push runs**.

- **Changed:** `forge verified-push-run` now promotes the normalized `live_status_evidence` already validated by push readiness into its top-level result. The requested commit, bounded workflow-run limit, collection-completeness proof, and per-run commit-binding proof are directly reviewable without reopening nested push-readiness JSON.
- **Why:** AUTO-208 preserved the proof through push readiness, but higher-level verified-push consumers still had to traverse nested handoff evidence. AUTO-209 closes that reviewability gap without creating a second provenance contract.
- **Validation:** AUTO-208 baseline head `5b5af577487cb86ba5df764455abb7cc5f57e7be` is green in Actions run 32925219891. AUTO-209 adds deterministic coverage for live provenance promotion and backward-compatible non-live behavior; final-head CI is inspected before completion is claimed.
- **Safety:** no new external command, network surface, workflow rerun, push authority, force-push/tag-push behavior, or remote/protection mutation was added. Existing readiness checks remain authoritative and `--confirm-push` stays independent.
...
- **Visual updates:** none; this improves evidence visibility inside the existing push stage without changing lifecycle topology.
- **Current limitations:** commit trust and branch protection remain caller-supplied; live status still depends on installed/authenticated `gh`; canonical durable maintenance evidence still retains this proof only through nested verified-push artifacts rather than a concise durable summary.
- **Next autonomous objective:** inspect AUTO-209 CI first; any failure takes priority. If green, carry this same normalized proof into a compact durable maintenance/history summary that reuses the existing field rather than introducing parallel semantics.
