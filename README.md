...
Forge supports SHA-linked maintenance bundles, run-history records, replay/reviewer handoffs, archive manifests, copied archive roots, `.tar`/`.tar.gz`/`.zip` packaging, package verification, preservation-completeness checks, and immutable preservation receipts. Durable maintenance bundle outputs, maintenance history links, preservation receipts, confirmed archive manifests, and primary run-history records refuse silent overwrite through their normal writers; a later run must choose a new output path rather than clobber preserved evidence. Maintenance bundle publication, maintenance history-link publication, archive-manifest publication, and primary run-history publication now use flushed same-directory temporary files, atomic no-clobber hard-link publication, and parent-directory `fsync`, closing the race between output preflight and the final durable write.
...
Latest stewardship run: **AUTO-186 — harden durable maintenance evidence persistence against races**.

- **Changed:** confirmed maintenance-bundle and maintenance-history-link publication now use same-directory temporary files, flush + file `fsync`, atomic no-clobber hard-link publication, parent-directory `fsync`, and temporary-file cleanup instead of direct `Path.write_text()`.
- **Why:** both writers already refused an existing output at preflight, but another process could create the target between that check and the final direct write. AUTO-186 closes that time-of-check/time-of-use overwrite window at both durable maintenance-evidence boundaries.
- **Safety:** bundle completeness, explicit bundle/history-link confirmations, repository and `.ai/run-history/` containment, JSON-extension enforcement, and existing-output refusal remain unchanged. If a racing writer wins, Forge returns a blocked result and preserves the competing bytes. No network, validation execution, workflow, Git, remote, force-push, protection, or overwrite authority was added.
...
- **Validation:** deterministic regression coverage verifies racing-writer refusal for both bundle and history-link publication, preservation of competing bytes, temporary-file cleanup, and file/directory fsync on success. Full checkout/full pytest remains unavailable because this runtime cannot resolve `github.com`; no green Python 3.10/3.11/3.12 claim is made until final-head CI is observable.
- **Visual updates:** none; this is a durability correction at existing persistence boundaries and the lifecycle diagram remains accurate.
- **Current limitations:** no-clobber publication relies on normal same-filesystem hard-link support; Forge still does not provide a shared inter-process lock, merge competing evidence records, or prove signer identity/validation sufficiency.
- **Next autonomous objective:** inspect AUTO-186 CI when observable. Any failure takes priority; if green, continue only with another concrete end-to-end persistence/provenance integrity defect or a meaningful reduction in caller-managed handoffs.
