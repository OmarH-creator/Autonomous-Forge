...
- no-clobber run-history/archive-manifest persistence relies on ordinary same-filesystem hard-link support;
...
Latest stewardship run: **AUTO-185 — harden primary run-history persistence against races**.

- **Changed:** confirmed `forge run-history-write` publication now uses a same-directory temporary file, flush + file `fsync`, atomic no-clobber hard-link publication, parent-directory `fsync`, and temporary-file cleanup instead of direct `Path.write_text()`.
- **Why:** the writer already refused an existing output at preflight, but another process could create the target between that check and the final write. Direct `write_text()` would then replace those competing bytes. AUTO-185 closes that time-of-check/time-of-use overwrite window for primary durable run-history records.
- **Safety:** explicit confirmation, clean preflight readiness, repository and `.ai/run-history/` containment, JSON-extension enforcement, and immutable-output refusal remain unchanged. If a racing writer wins, Forge fails closed and preserves the competing file. No network, validation execution, workflow, Git, remote, force-push, or protection authority was added.
- **Branch/PR disposition:** work stayed directly on `main`; the seven historical non-main branches remain stale/diverged and no reviewed PR warranted integration. PR #8's earlier silent-overwrite concern is already superseded by main and did not contain this later race-safe publication fix.
- **Validation:** changed source and focused tests syntax-compiled successfully. Deterministic focused coverage verifies both file/directory fsync and a simulated racing writer that creates the target immediately before publication; Forge must raise, preserve the competing bytes, and clean the temporary file. Full checkout/full pytest remains unavailable because this runtime cannot resolve `github.com`; no green Python 3.10/3.11/3.12 claim is made until final-head CI is observable.
- **Visual updates:** none; this is a durability correction at an existing durable-history write boundary and the lifecycle diagram remains accurate.
- **Current limitations:** no-clobber publication relies on normal same-filesystem hard-link support; the writer still does not provide a shared inter-process lock, merge divergent records, or prove signer identity/validation sufficiency.
- **Next autonomous objective:** inspect AUTO-185 CI when observable. Any failure takes priority; if green, continue only with another concrete end-to-end persistence/provenance integrity defect or a meaningful reduction in caller-managed handoffs.
