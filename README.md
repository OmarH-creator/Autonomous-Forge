...
Autonomous Forge is pre-alpha. Latest autonomous run: AUTO-082 shipped `forge patch-application-readiness` and compatibility `forge-patch-application-readiness`, a read-only summary that combines ready patch-application preflight JSON with clear patch-application audit JSON before any future patch-applier design. It verifies matching objectives, reviewed paths, validation steps, upstream blockers, and the invariant that patch application remains disallowed. Direct local checkout/test execution remains unavailable in this environment, so validation was limited to static GitHub API review plus deterministic tests committed for the new core, CLI, and installed router paths; no final workflow/status run was visible yet. No visual updates were needed because the overview diagram still accurately describes the read-only workflow boundary. Next objective: add installed CI smoke coverage for the new readiness command, then continue toward a guarded patch-applier design only if readiness evidence remains clear.

The repository now contains:

- Apache-2.0 licensing and durable planning files in `.ai/`.
- A minimal Python package with a primary `forge` console script and compatibility `forge-patch-application-audit` / `forge-patch-application-preflight` / `forge-patch-application-readiness` / `forge-patch-proposal-review` / `forge-patch-proposal-draft` / `forge-patch-text-preflight` / `forge-patch-text-review` console scripts.
...
- Smoke and deterministic coverage for the CLI’s current local workflows, including primary and compatibility patch proposal review/draft behavior, primary/compatibility patch text preflight behavior, primary/compatibility patch text review behavior, primary/compatibility patch application preflight behavior, patch application audit behavior, and patch application readiness behavior.
