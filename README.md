...
Autonomous Forge is pre-alpha. Latest autonomous run: AUTO-103 added `forge maintenance-replay-summary`, a read-only replay decision for persisted maintenance evidence bundles. The command verifies source-report hashes through the existing bundle verifier, checks that the saved bundle is complete, confirms the expected patch/validation/commit/push/post-push evidence stages, and reports whether the recorded maintenance chain is still replayable. Direct full checkout/test execution remains unavailable in this environment; validation included static GitHub API review of the new module, CLI, tests, package script, workflow smoke, README, docs, and project-memory updates. No visual updates were needed because the existing overview still accurately describes the safety boundary. Current limitations: replay summaries do not rerun workflows, poll remote status, verify branch protection, enforce maintainer identity allowlists, or prove cryptographic trust beyond recorded evidence. Next autonomous objective: add maintainer allowed-signer policy support for commit trust evidence.
...
- Smoke and deterministic coverage for the CLI’s current local workflows, including primary and compatibility replay summary behavior for complete and blocked persisted bundles.
...
forge maintenance-evidence-bundle --root . --patch-apply patch-apply.json --post-apply-validation post-apply-validation.json --commit-verify commit-verify.json --push-handoff push-handoff.json --post-push-verify post-push-verify.json --bundle-id AUTO-103 --require-complete --format json > maintenance-evidence-bundle.json
forge maintenance-evidence-bundle --root . --patch-apply patch-apply.json --post-apply-validation post-apply-validation.json --commit-verify commit-verify.json --push-handoff push-handoff.json --post-push-verify post-push-verify.json --bundle-id AUTO-103 --output .ai/run-history/AUTO-103-bundle.json --confirm-write --require-written --format json
forge maintenance-bundle-verify --root . --bundle .ai/run-history/AUTO-103-bundle.json --require-verified --format json > maintenance-bundle-verify.json
forge maintenance-replay-summary --root . --bundle .ai/run-history/AUTO-103-bundle.json --require-replayable --format json > maintenance-replay-summary.json
