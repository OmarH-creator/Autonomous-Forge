...
Autonomous Forge is pre-alpha. Latest autonomous run: AUTO-105 added optional maintainer allowed-signer policy support to `forge commit-trust-review`. The trust gate can now read one repository-local JSON policy and block otherwise trusted signatures when the inspected signer/key fingerprint is not allowlisted; this strengthens the push-readiness chain without adding commit, push, network, remote-configuration, workflow-rerun, or branch-protection authority. Validation included static GitHub API review, scratch syntax compilation for the updated module/CLI/tests, and a focused scratch pytest run of `tests/test_commit_trust_review.py`, which passed 8 tests. No visual updates were needed because the existing overview still accurately describes the local-first safety boundary. Current limitations: direct full checkout/full pytest execution and final workflow visibility remain unavailable from this environment; the product still lacks branch-protection verification, remote workflow rerun/polling, and signed evidence attestation. Next autonomous objective: add branch-protection/status-policy inspection before push handoff.
...
- Smoke and deterministic coverage for the CLI’s current local workflows, including primary and compatibility replay summary behavior, fast-forward push-handoff behavior, and allowed-signer commit-trust behavior for matching, mismatched, malformed, unsigned, and bad-signature cases.
...
forge maintenance-evidence-bundle --root . --patch-apply patch-apply.json --post-apply-validation post-apply-validation.json --commit-verify commit-verify.json --push-handoff push-handoff.json --post-push-verify post-push-verify.json --bundle-id AUTO-105 --require-complete --format json > maintenance-evidence-bundle.json
forge maintenance-evidence-bundle --root . --patch-apply patch-apply.json --post-apply-validation post-apply-validation.json --commit-verify commit-verify.json --push-handoff push-handoff.json --post-push-verify post-push-verify.json --bundle-id AUTO-105 --output .ai/run-history/AUTO-105-bundle.json --confirm-write --require-written --format json
forge maintenance-bundle-verify --root . --bundle .ai/run-history/AUTO-105-bundle.json --require-verified --format json > maintenance-bundle-verify.json
forge maintenance-replay-summary --root . --bundle .ai/run-history/AUTO-105-bundle.json --require-replayable --format json > maintenance-replay-summary.json
