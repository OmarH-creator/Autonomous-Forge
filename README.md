...
The project starts as a local-first Python CLI. Its first goal is deliberately small: provide a `forge` command that can grow into dry-run planning, task selection, validation reporting, diff review, supplied commit-status review, combined change readiness, guarded patch preview, explicitly confirmed patch application, post-apply validation handoff, commit-readiness review, commit metadata preview, explicitly confirmed local commit creation, post-commit verification, push-readiness review, an explicitly confirmed non-force push handoff, post-push verification, durable maintenance evidence bundles, and durable repository memory without requiring uncontrolled autonomous behavior.

For a visual orientation to the current workflow and its safety boundary, see [the project overview](docs/OVERVIEW.md).

## Current Autonomous Status

Autonomous Forge is pre-alpha. Latest autonomous run: AUTO-099 added `forge maintenance-evidence-bundle` and compatibility `forge-maintenance-evidence-bundle`, an end-to-end evidence bundling command that links completed patch-apply, post-apply validation, commit verification, pushed handoff, and post-push verification reports. It checks that the same reviewed paths and commit flow through the completed chain, reports blockers for stale or inconsistent evidence, and can persist one bounded JSON bundle only with `--output` plus `--confirm-write`. Direct full checkout/test execution remains unavailable in this environment; validation included scratch syntax compilation plus focused pytest for the new maintenance evidence bundle tests, static GitHub API review, and CI smoke coverage for the new primary and compatibility help routes. No visual updates were needed because the existing overview still accurately describes the safety boundary. Current limitations: no signed commit verification, cryptographic trust model, automatic validation execution after patch application, or remote workflow rerun/polling. Next autonomous objective: add hash-linked evidence bundle integrity checks so durable bundles can detect stale or mismatched source reports.
...
- Smoke and deterministic coverage for the CLI’s current local workflows, including primary and compatibility maintenance evidence bundle behavior.
...
forge maintenance-evidence-bundle --root . --patch-apply patch-apply.json --post-apply-validation post-apply-validation.json --commit-verify commit-verify.json --push-handoff push-handoff.json --post-push-verify post-push-verify.json --bundle-id AUTO-099 --require-complete --format json > maintenance-evidence-bundle.json
forge maintenance-evidence-bundle --root . --patch-apply patch-apply.json --post-apply-validation post-apply-validation.json --commit-verify commit-verify.json --push-handoff push-handoff.json --post-push-verify post-push-verify.json --bundle-id AUTO-099 --output .ai/run-history/AUTO-099-bundle.json --confirm-write --require-written --format json
