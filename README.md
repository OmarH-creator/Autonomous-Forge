...
- preservation-receipt discovery is intentionally informational and does not make a receipt mandatory for an otherwise complete preservation artifact;
...
Latest stewardship run: **AUTO-180 — discover verified preservation receipts in the review path**.

- **Changed:** extended the existing `forge maintenance-preservation-receipt` command with `--discover`. Given one preservation-completeness artifact, Forge independently verifies that artifact is complete, performs a deterministic bounded scan of `.ai/preservation-receipts/*.json`, verifies matching receipts, ignores valid receipts bound to other completeness artifacts, and surfaces malformed or drifted candidates for attention.
- **Safety:** discovery is read-only and scans at most 100 direct JSON candidates. It rejects a symlinked receipt directory and fixes `receipt_gate_effect=informational_only` plus `receipt_required_for_preservation=false`, so receipt existence, absence, or review status cannot replace the established preservation-completeness gate. Receipt persistence remains behind its separate `--confirm-write` authority.
- **Branch/PR disposition:** work stayed on `main`. The seven historical non-main branches remain stale/diverged; reviewed PRs are merged/closed/obsolete or unrelated and none warranted integration.
- **Validation:** the changed receipt core, CLI, and focused tests syntax-compile in the available scratch environment. A focused executable smoke passed no-receipt review, confirmed receipt persistence/discovery, matching receipt verification, and tampered matching-receipt attention behavior while preserving `preservation_complete=true`. Full checkout pytest remains unavailable because this runtime cannot resolve `github.com`; the connected status surface exposed no checks for AUTO-179 at run start, so no unsupported green-matrix claim is made.
- **Visual updates:** none; discovery is a reviewer-facing read on the existing terminal preservation edge, so the lifecycle diagram remains accurate.
- **Current limitations:** receipt discovery proves hash continuity only for matching receipts and does not prove signer identity, validation sufficiency, or require receipts for preservation. Malformed files in the dedicated receipt directory are surfaced for attention but do not change the independently established completeness result.
- **Next autonomous objective:** inspect AUTO-180 CI when observable; if green, carry verified receipt discovery into a higher-level preservation review/comparison surface only when it avoids duplicate evidence contracts and preserves informational-only semantics.
