...
- live workflow-status collection depends on a functioning authenticated GitHub CLI, requires every admitted run to report the exact requested commit as its head SHA, applies a 15-second timeout to each live `git`/`gh` subprocess, refuses a result set that exceeds the configured 20-run review window rather than treating truncated workflow evidence as complete, and now requires those completeness/commit-binding guarantees to survive the payload→review→push-readiness→verified-push-run→durable-maintenance→maintenance-history-link→linked-bundle-review conversion; it still does not rerun workflows or prove that the configured checks are sufficient;
...
Latest stewardship run: **AUTO-212 — verify live-status provenance during linked history review**.

- **Changed:** `forge maintenance-history-link-review --verify-linked-bundle` now verifies a present compact `live_status_evidence_summary` against the authoritative linked bundle. It recomputes the normalized live-status SHA-256 and checks source, exact commit, bounded workflow-run limit, collection completeness, per-run commit binding, and field-for-field continuity with `verified_provenance.live_status_evidence`.
- **Why:** AUTO-211 made live-status proof discoverable in compact run history, but the linked reviewer still verified only the older external-validation summary. AUTO-212 closes that evidence boundary so a tampered compact live-status summary cannot survive linked-bundle review.
- **Validation:** final product/status head `5100252f76860c963e805f5dc27420a07bb82fa3` passed GitHub Actions run `33003643413`. Python 3.10, 3.11, and 3.12 each passed checkout/install, source compilation, installed CLI smoke tests, roadmap validation, and pytest.
- **Safety:** evidence verification only. No new network/subprocess surface, workflow rerun, push authority, force-push/tag-push behavior, remote mutation, branch-protection mutation, or persistence authority was added. Legacy history links without live-status summaries remain compatible.
...
- **Visual updates:** none; this extends verification through an existing evidence boundary without changing lifecycle topology.
- **Current limitations:** the linked durable bundle remains authoritative; this verifies provenance continuity, not workflow sufficiency. Commit trust and branch protection remain caller-supplied.
- **Next autonomous objective:** carry the verified linked live-status result into higher-level maintenance review handoff/comparison output only where it improves reviewability without duplicating the evidence contract or changing readiness/ranking semantics.
