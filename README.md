...
Latest stewardship run: **AUTO-183 — enforce preservation receipt deduplication in the comparison core**.

- **Changed:** `build_maintenance_review_compare_data()` now canonicalizes and deduplicates `completeness_paths` against `root` before receipt discovery. The evidence-accounting guarantee introduced at the AUTO-182 CLI boundary therefore also applies to direct Python callers and any future command surface reusing the comparison builder.
- **Safety:** the change only narrows duplicate evidence accounting. Receipt review remains `informational_only`, cannot change comparison readiness or preservation ranking, and grants no write, Git, workflow, remote, network, validation, or preservation authority.
...
- **Validation:** focused deterministic coverage calls the comparison builder directly with equivalent relative, dotted-relative, and absolute paths and asserts one receipt review, one verified receipt count, and one candidate match. The canonicalization helper was also exercised independently. Full checkout pytest remains unavailable because this runtime cannot resolve `github.com`; the connected status surface exposed no AUTO-182 checks, so no unsupported green-matrix claim is made for AUTO-183 until CI becomes observable.
- **Visual updates:** none; this is an integrity correction inside the existing preservation-review path and the lifecycle diagram remains accurate.
- **Current limitations:** receipt evidence remains informational and proves byte continuity, not signer identity or validation sufficiency. The comparison still trusts the existing preservation-completeness/receipt verification contracts for semantic correctness.
- **Next autonomous objective:** inspect AUTO-183 CI when observable. Any failure takes priority; if green, continue only with a concrete preservation-review or end-to-end integrity defect rather than another parallel evidence contract.
