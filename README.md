...
Latest stewardship run: **AUTO-189 — harden archive-copy publication and source-integrity continuity**.

- **Changed:** confirmed `maintenance-archive-copy` now copies every manifest entry into a same-directory temporary file, recomputes byte count and SHA-256 against the verified copy preview, file-fsyncs the verified temporary copy, atomically publishes it with a no-clobber hard link, parent-directory-fsyncs the publication, and removes the temporary name.
- **Why:** the previous `shutil.copy2(source, destination)` path ran after an earlier existence check. A competing process could create the destination after preflight and be overwritten, while source bytes could drift after preview and still be published before later verification detected the mismatch.
- **Safety:** written-manifest verification, copy-preview readiness, explicit `--confirm-copy`, repository containment, destination-collision checks, and optional parent creation remain unchanged. Racing destinations are preserved, source drift blocks before final publication, and no network, workflow, Git remote, force-push, protection, or overwrite authority was added.
...
- **Validation:** changed source/tests syntax-compile in the available Python environment. Deterministic coverage now exercises racing-writer refusal with competing-byte preservation, source-drift refusal before publication, file/directory fsync, existing-output refusal, normal copying, and CLI behavior. Full checkout/full pytest remains unavailable because this runtime cannot resolve `github.com`; no green Python 3.10/3.11/3.12 claim is made until final-head CI is observable.
- **Visual updates:** none; this is a durability/integrity correction at the existing archive-copy boundary and the lifecycle diagram remains accurate.
- **Current limitations:** no-clobber publication relies on normal same-filesystem hard-link support; archive-copy publication is per-file rather than a cross-file transaction, so a later-entry failure can leave earlier entries durably published for inspection; a directory-fsync failure after link publication requires inspection before retrying.
- **Next autonomous objective:** inspect AUTO-189 CI when observable. Any failure takes priority; if green, continue only with another concrete end-to-end persistence/provenance integrity defect or a meaningful reduction in caller-managed handoffs.