...
- atomic patch replacement preserves the target permission mode but intentionally does not promise preservation of every filesystem metadata field; a parent-directory fsync failure after replacement means the target has already changed and must be inspected before retrying;
...
Latest stewardship run: **AUTO-190 — make guarded patch application crash-safe and rollback-safe**.

- **Changed:** confirmed guarded patch writes now build the replacement in a same-directory temporary file, preserve the target permission mode, flush and fsync the complete bytes, atomically switch the target with `os.replace`, fsync the containing directory, and clean up the temporary name. Failed live-diff verification restores the original text through the same atomic path.
- **Why:** `_apply_prepared_patch()` previously used `Path.write_text()` for both apply and rollback. That truncates the existing target in place, so an interruption or write error could expose a partially written working file before Forge reached live-diff verification or rollback.
- **Safety:** preview reproduction checks, ready change evidence, explicit `--confirm-apply`, path/symlink containment, simple secret-marker checks, target-scoped policy-aware live-diff verification, and rollback behavior remain intact. No network, new external command, workflow, Git remote, force-push, branch-protection, or persistence authority was added.
- **Branch/PR disposition:** work stayed directly on `main`; historical non-main branches remain old/diverged and no reviewed PR warranted integration.
- **Validation:** deterministic coverage now exercises atomic replacement success, permission-mode preservation, temporary cleanup, failed replacement preserving original bytes, file/directory fsync, and truthful reporting when directory fsync fails after replacement. Full checkout/full pytest remains unavailable because this runtime cannot resolve `github.com`; no green Python 3.10/3.11/3.12 claim is made until final-head CI is observable.
- **Visual updates:** none; this hardens the existing guarded patch-apply boundary and the lifecycle diagram remains accurate.
- **Current limitations:** atomic replacement changes the target inode and does not promise preservation of every metadata field; a parent-directory fsync failure after successful replacement means the target has already changed and requires inspection before retrying; Forge still has no shared cross-process target lock.
- **Next autonomous objective:** inspect AUTO-190 CI when observable. Any failure takes priority; if green, continue only with another concrete mutation/persistence integrity defect or a meaningful reduction in caller-managed handoffs.
