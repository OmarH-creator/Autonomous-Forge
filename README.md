...
Latest stewardship run: **AUTO-178 — expose advisory validation provenance at final preservation completeness**.

- **Changed:** `forge maintenance-preservation-completeness` now exposes `external_validation_provenance` at the final preservation boundary, including presence, verification state, attachment count, evidence SHA-256, fixed advisory semantics, and cross-layer continuity checks between the written manifest, copied-root verification, and package verification.
- **Safety:** externally supplied observations remain `externally_supplied_observation`, `executor_validation_equivalent: false`, `bundle_gate_effect: advisory_only`, and `preservation_gate_effect: none`. Provenance continuity is reviewer information only; it cannot make preservation complete or incomplete, satisfy executor validation, or promote external evidence into Forge-executed proof.
- **Branch/PR disposition:** work stayed on `main`. Every visible historical branch was compared with current `main` and is hundreds to more than 1,600 commits behind; reviewed PRs are merged/closed/obsolete or unrelated and none warranted integration.
- **Validation:** the changed core and focused AUTO-178 tests syntax-compile in the available execution environment. Direct checkout/full pytest remains unavailable because this runtime cannot resolve `github.com`. The connected GitHub status/workflow surfaces expose no AUTO-177 result, so no unsupported green-matrix claim is carried forward.
- **Visual updates:** none; AUTO-178 only carries verified metadata through the existing final preservation edge, so the lifecycle diagram remains accurate.
- **Current limitations:** SHA-256 continuity detects evidence-byte drift but does not prove signer identity. Legacy manifests without advisory provenance remain compatible and report `status=not_present`. Cross-layer advisory-provenance drift is surfaced to reviewers but deliberately remains non-gating.
- **Next autonomous objective:** inspect AUTO-178 CI when observable; if green, add a durable preservation receipt only if it can reuse the same verified completeness artifact without weakening independent persistence authority or creating a duplicate evidence contract.
