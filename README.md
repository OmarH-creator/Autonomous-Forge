...
Autonomous Forge is pre-alpha. Latest autonomous run: AUTO-119 made `forge maintenance-replay-summary` enforce validation-context consistency before a persisted bundle can be trusted as replayable. Replay summaries now compare retained `expected_file_changes` against reviewed paths and retained `validation_steps` against bundle validation steps, then block replayability when that retained implementation context no longer matches the reviewed evidence chain. Validation included static source/test/docs review through the GitHub repository API; direct full repository checkout/full pytest remains unavailable from this environment. No visual updates were needed because the existing overview still accurately represents the local-first workflow boundary. Current limitations: consistency checks compare advisory JSON evidence and do not prove validation actually exercised every planned file, step, or risk. Next autonomous objective: add a compact bundle replay policy summary that shows which replay gates passed, failed, or remained advisory.
