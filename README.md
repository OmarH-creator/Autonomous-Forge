...
- immutable validation sidecars are visible through `run-history-read`, replay, durable bundle construction, and maintenance history-link summaries, but remain externally supplied observations and are never promoted to executor-produced validation evidence;
...
Latest stewardship run: **AUTO-173 — summarize immutable external validation provenance in maintenance history links**.

- **Changed:** maintenance history-link persistence now emits `external_validation_evidence_summary` when the written bundle carries verified external validation provenance. The compact pointer preserves the source record label, attachment count, advisory/non-executor semantics, and a deterministic SHA-256 of the full `external_validation_evidence` block.
- **Safety:** the history-link writer fails closed if external provenance attempts to become executor-equivalent, changes its bundle effect away from `advisory_only`, has inconsistent attachment counts, malformed attachment hashes/byte counts, or cannot be serialized deterministically. Bundles without external validation evidence keep the legacy history-link shape.
...
- **Validation:** deterministic AUTO-173 coverage verifies hash-bound summary persistence, refusal of executor-proof promotion, and backward-compatible link shape. Direct full-checkout pytest remains unavailable because this runtime cannot resolve `github.com`; the pushed head's GitHub workflow/status is inspected separately and no green matrix claim is made without evidence.
- **Visual updates:** none; this strengthens the existing durable-evidence pointer and does not change the lifecycle architecture.
- **Current limitations:** the compact SHA-256 is an integrity/index pointer, not signer identity. Consumers still need the full bundle to inspect individual attachment paths, source hashes, notes, and retained validation context. Fresh commit-trust/status/protection acquisition remains policy-gated.
- **Next autonomous objective:** inspect AUTO-173 CI when observable; if green, teach the existing history-link review/replay surfaces to validate and expose the compact advisory-provenance summary without treating it as executor-produced validation proof.