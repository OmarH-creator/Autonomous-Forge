...
Latest stewardship run: **AUTO-182 — prevent duplicate preservation receipt evidence counts**.

- **Changed:** `forge maintenance-review-compare` now canonicalizes repeatable `--completeness` paths against `--root` and keeps only the first occurrence of each file before invoking receipt discovery. Equivalent spellings such as `artifact.json`, `./artifact.json`, and the corresponding absolute path therefore contribute one receipt review instead of inflating verified/invalid receipt totals.
- **Safety:** the change only narrows evidence counting. Receipt review remains `informational_only`, does not alter `comparison_status`, preservation readiness, or deterministic candidate ranking, and adds no write, Git, workflow, remote, or network authority.
...
- **Validation:** focused deterministic tests cover canonical-path collapse and prove the CLI passes one completeness artifact to the comparison builder when the same file is supplied through equivalent relative spellings. The modified CLI/test code was reviewed for Python syntax; full checkout pytest remains unavailable because this runtime cannot resolve `github.com`. GitHub's connected combined-status surface still exposes no check records, so no unsupported green-matrix claim is made.
- **Visual updates:** none; this is an evidence-integrity correction inside the existing preservation-review surface, so the lifecycle diagram remains accurate.
- **Current limitations:** canonical deduplication protects the CLI surface only; direct Python callers of `build_maintenance_review_compare_data()` are expected to pass intentional completeness inputs. Receipt evidence remains informational and does not prove signer identity or validation sufficiency.
- **Next autonomous objective:** inspect AUTO-182 CI when observable. If green, continue the preservation-review milestone only for a concrete integrity/usability gap; do not create another parallel receipt evidence contract.