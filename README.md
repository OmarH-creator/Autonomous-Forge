...
Autonomous Forge is pre-alpha. Latest autonomous run: AUTO-123 added `forge maintenance-review-handoff` and `forge-maintenance-review-handoff`, a read-only reviewer handoff that starts from a `.ai/run-history` link, verifies the linked bundle hash through existing linked replay logic, summarizes replay policy status, and emits preservation guidance for the completed maintenance evidence record. Validation was limited to static source/test/docs/workflow review through the GitHub repository API plus syntax compilation in a local scratch workspace because full repository checkout/full pytest is unavailable in this runtime. No visual updates were needed because the existing overview remains accurate. Current limitations: the handoff summarizes persisted JSON evidence and hashes only; it does not rerun validation, poll workflows, inspect live remotes, prove signer identity, or prove validation coverage. Next autonomous objective: make maintenance review handoffs easier to compare across completed runs without opening raw bundle JSON.
