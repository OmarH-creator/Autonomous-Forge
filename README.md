...
Autonomous Forge is pre-alpha. Latest autonomous run: AUTO-072 hardened the GitHub Actions workflow so the documented primary `forge patch-proposal-review` command is exercised in the installed end-to-end smoke path. CI now also runs the compatibility `forge-patch-proposal-review` command for the same ready evidence, JSON-validates both outputs, and asserts they are identical. This closes a workflow risk where the standalone compatibility script could keep passing while the primary `forge` route regressed. Direct local checkout/test execution remained unavailable in this environment, so validation was limited to GitHub API static review plus committed workflow assertions. No visual updates were needed because the current overview diagram remains accurate. Next objective: add the first read-only patch proposal draft preview from ready proposal-review evidence without generating or applying patches.
...
- Smoke and deterministic coverage for the CLI’s current local workflows, including primary and compatibility patch proposal review behavior.
- CI smoke coverage that validates live repository roadmap, policy, state, installed console entry points, the primary `forge patch-proposal-review` route, the compatibility `forge-patch-proposal-review` route, matching JSON between both proposal-review routes, run-history persistence/list/latest flow, validation-result handoff, and executor-observation audit behavior.
