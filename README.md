...
- immutable validation sidecars are visible through `run-history-read`, replay, durable bundle construction, maintenance history-link summaries, linked-bundle history review, and reviewer handoff/comparison surfaces, but remain externally supplied observations and are never promoted to executor-produced validation evidence;
...
Latest stewardship run: **AUTO-175 — preserve verified external validation provenance in reviewer handoff/comparison surfaces**.

- **Changed:** `forge maintenance-review-handoff` now exposes a compact `external_validation_provenance` summary derived from AUTO-174 linked-bundle verification. `forge maintenance-review-compare` preserves the same summary in each handoff and preservation candidate and reports how many compared handoffs contain verified advisory validation evidence.
- **Safety:** external observations remain `externally_supplied_observation`, `executor_validation_equivalent: false`, and `bundle_gate_effect: advisory_only`. They do not add a handoff gate, cannot make a blocked run ready, and are deliberately excluded from preservation ranking so externally supplied evidence cannot boost a candidate.
...
- **Validation:** deterministic AUTO-175 coverage checks handoff propagation, comparison/candidate propagation, stable text exposure, and preservation-score neutrality. Direct full-checkout pytest remains unavailable because this runtime cannot resolve `github.com`; final GitHub status/workflow evidence is inspected separately and no green matrix claim is made without observable evidence.
- **Visual updates:** none; this carries already-verified provenance across existing reviewer surfaces and does not change the lifecycle architecture.
- **Current limitations:** SHA-256 continuity proves consistency with linked bundle provenance bytes, not signer identity. Legacy links without the compact external-validation summary remain compatible and surface `status=not_present` rather than verified provenance.
- **Next autonomous objective:** inspect AUTO-175 CI when observable; if green, carry the same advisory-provenance visibility into preservation/archive manifest output so preserved evidence sets retain this reviewer-facing provenance summary without reopening comparison JSON.
