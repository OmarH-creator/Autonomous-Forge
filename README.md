...
After all retained validation commands pass, verified change orchestration hashes the exact target bytes into commit-readiness evidence. Verified commit creation re-hashes that target immediately before staging and refuses the commit if the file changed after validation. Forge now performs verified staging through a private temporary Git index initialized from the reviewed `HEAD`, so unrelated caller staging cannot enter the commit. Reviewed paths already staged in the shared index are refused; after a verified commit Forge confirms those shared-index entries did not change concurrently before synchronizing only the reviewed paths to the new `HEAD`, preserving unrelated staged work. Within the private index, Forge hashes the exact staged bytes for the target, requires the complete staged path set to equal the reviewed paths, rechecks the reviewed parent `HEAD`, and repeats both index checks immediately before `git commit`. After Git creates the commit, Forge verifies the target bytes, exact changed paths, and created commit parent before the commit can be marked verified.
...
- verified commit creation now stages through a private temporary Git index, but the branch ref update is not compare-and-swap and the post-commit synchronization of reviewed shared-index entries is a separate Git transaction. Forge snapshots those entries and refuses automatic synchronization if they changed concurrently; existing exact parent/target/path post-commit checks remain defense in depth for concurrent `HEAD` movement;
...
Latest stewardship run: **AUTO-198 — isolate verified commit staging from the shared Git index**.

- **Changed:** both `forge verified-commit-create` and the higher-level verified-change orchestration now stage through a private temporary Git index initialized from the reviewed `HEAD`. Unrelated caller staging cannot enter the Forge commit. Reviewed paths already staged by the caller are refused; after a verified commit Forge confirms those shared-index entries did not change concurrently before synchronizing only the reviewed paths to the new `HEAD`.
- **Why:** AUTO-197 narrowed shared-index races with a final pre-commit revalidation, but the commit path still relied on the repository's shared staging area. That allowed unrelated caller/agent staging to contend with Forge and left a tiny mutation window around the final check.
- **Safety:** existing validation-byte binding, staged-byte/path verification, final pre-commit index revalidation, reviewed-parent binding, Git hooks, post-commit target/parent/path verification, explicit commit confirmation, non-force push behavior, and no remote/protection mutation guarantees remain intact. Shared-index synchronization is fail-closed if reviewed entries changed concurrently, and unrelated staged work is preserved.
...
- **Validation:** deterministic AUTO-198 coverage uses a disposable real Git repository to prove the created commit contains only the reviewed target while unrelated shared-index staging survives. Additional tests cover refusal of already-staged reviewed paths, no Git invocation without commit confirmation, private-index cleanup, and initialization failure. A focused local Git experiment independently confirmed the private-index commit plus reviewed-path synchronization semantics. Direct clone/full pytest remains unavailable because this runtime cannot resolve `github.com`; the Python 3.10/3.11/3.12 matrix is not claimed green without observable CI evidence.
- **Visual updates:** none; this strengthens the existing verified-commit stage and does not change lifecycle topology.
- **Current limitations:** private index isolation removes ordinary contention with unrelated shared staging but is not a compare-and-swap branch-ref update. Shared-index synchronization after a verified commit is a separate Git transaction; Forge snapshots reviewed entries and refuses the sync if they changed concurrently.
- **Next autonomous objective:** inspect AUTO-198 CI when observable; any failure takes priority. If green, continue the integrated maintenance milestone only with another concrete cross-stage integrity defect or meaningful evidence-handoff reduction.
