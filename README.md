...
The project starts as a local-first Python CLI. Its first goal is deliberately small: provide a `forge` command that can grow into dry-run planning, task selection, validation reporting, diff review, supplied commit-status review, combined change readiness, guarded patch preview, explicitly confirmed patch application, post-apply validation handoff, commit-readiness review, commit metadata preview, explicitly confirmed local commit creation, post-commit verification, push-readiness review, an explicitly confirmed non-force push handoff, post-push verification, hash-linked durable maintenance evidence bundles, persisted bundle verification, and durable repository memory without requiring uncontrolled autonomous behavior.

For a visual orientation to the current workflow and its safety boundary, see [the project overview](docs/OVERVIEW.md).

## Current Autonomous Status

Autonomous Forge is pre-alpha. Latest autonomous run: AUTO-101 added `forge maintenance-bundle-verify`, a read-only persisted bundle verification command that recomputes the byte count and SHA-256 digest for each repository-local source report recorded in a maintenance evidence bundle. This advances the durable evidence milestone beyond bundle creation by letting maintainers detect drift after a bundle has been persisted. Direct full checkout/test execution remains unavailable in this environment; validation included scratch syntax compilation plus focused scratch pytest for `tests/test_maintenance_bundle_verify.py` with 6 passing tests, static GitHub API review, and documentation review. No visual updates were needed because the existing overview still accurately describes the safety boundary. Current limitations: no signed commit verification, cryptographic trust model, automatic validation execution after patch application, or remote workflow rerun/polling. Next autonomous objective: add a compact signed-or-authority-aware commit trust checkpoint before push-readiness, or improve local end-to-end evidence replay if signature data is unavailable.

The repository now contains:

- Apache-2.0 licensing and durable planning files in `.ai/`.
- A minimal Python package with a primary `forge` console script and compatibility `forge-change-readiness` / `forge-commit-create` / `forge-commit-proposal-preview` / `forge-commit-readiness` / `forge-commit-status-review` / `forge-commit-verify` / `forge-git-diff-review` / `forge-maintenance-bundle-verify` / `forge-maintenance-evidence-bundle` / `forge-patch-application-audit` / `forge-patch-application-preflight` / `forge-patch-application-readiness` / `forge-patch-apply` / `forge-patch-generation-preview` / `forge-patch-proposal-review` / `forge-patch-proposal-draft` / `forge-patch-text-preflight` / `forge-patch-text-review` / `forge-post-apply-validation` / `forge-post-push-verify` / `forge-push-handoff` / `forge-push-readiness` console scripts.
- Task parsing, deterministic task selection, roadmap linting, repository reports, policy summaries, run summaries, repository inventory, implementation plans, change proposals, validation plans, validation-run previews, validation orchestration previews, command-execution handoff previews, executor precondition gates, executor contract previews, executor dry-run previews, one narrow opt-in executor run command with explicit result-persistence handoff, guarded executor-handoff persistence, changed-file reviews, changed-content audit, supplied git-diff review, supplied or live commit/workflow status review, change readiness, guarded patch preview and patch apply, post-apply validation, commit readiness, guarded commit metadata preview, explicitly confirmed local commit creation, post-commit verification, push-readiness review, explicitly confirmed non-force push handoff, post-push verification, hash-linked durable maintenance evidence bundling, and persisted bundle source-report verification.
...
- Smoke and deterministic coverage for the CLI’s current local workflows, including primary and compatibility maintenance bundle verification behavior with source-report hash drift detection.
...
forge maintenance-evidence-bundle --root . --patch-apply patch-apply.json --post-apply-validation post-apply-validation.json --commit-verify commit-verify.json --push-handoff push-handoff.json --post-push-verify post-push-verify.json --bundle-id AUTO-101 --require-complete --format json > maintenance-evidence-bundle.json
forge maintenance-evidence-bundle --root . --patch-apply patch-apply.json --post-apply-validation post-apply-validation.json --commit-verify commit-verify.json --push-handoff push-handoff.json --post-push-verify post-push-verify.json --bundle-id AUTO-101 --output .ai/run-history/AUTO-101-bundle.json --confirm-write --require-written --format json
forge maintenance-bundle-verify --root . --bundle .ai/run-history/AUTO-101-bundle.json --require-verified --format json > maintenance-bundle-verify.json
