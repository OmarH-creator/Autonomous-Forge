...
The project starts as a local-first Python CLI. Its first goal is deliberately small: provide a `forge` command that can grow into dry-run planning, task selection, validation reporting, diff review, supplied commit-status review, combined change readiness, guarded patch preview, explicitly confirmed patch application, post-apply validation handoff, commit-readiness review, commit metadata preview, explicitly confirmed local commit creation, post-commit verification, commit trust review, push-readiness review, an explicitly confirmed non-force push handoff, post-push verification, hash-linked durable maintenance evidence bundles, persisted bundle verification, and durable repository memory without requiring uncontrolled autonomous behavior.

For a visual orientation to the current workflow and its safety boundary, see [the project overview](docs/OVERVIEW.md).

## Current Autonomous Status

Autonomous Forge is pre-alpha. Latest autonomous run: AUTO-102 added `forge commit-trust-review`, a local git signature/trust checkpoint that consumes verified commit evidence, inspects the same commit with `git show` signature metadata, and blocks unsigned, bad, expired, revoked, uncheckable, or mismatched commits before push-readiness can rely on them. This moves beyond evidence review by inspecting actual local git commit trust metadata while preserving the no-stage/no-commit/no-push safety boundary. Direct full checkout/test execution remains unavailable in this environment; validation included scratch syntax compilation, focused scratch pytest for `tests/test_commit_trust_review.py` with 5 passing tests, static GitHub API review, and documentation/workflow review. No visual updates were needed because the existing overview still accurately describes the safety boundary. Current limitations: trust is based on local git signature metadata only, not a full cryptographic identity policy, remote workflow rerun/polling, or branch-protection verification. Next autonomous objective: integrate commit-trust-review into push-readiness so ready pushes can require both verified commit content and trusted signature metadata.
...
- Smoke and deterministic coverage for the CLI’s current local workflows, including primary and compatibility commit trust review behavior with signed/unsigned/mismatched metadata cases.
...
forge commit-trust-review --root . --commit-verify commit-verify.json --format json > commit-trust-review.json
...
forge push-readiness --root . --commit-verify commit-verify.json --status-review verified-commit-status-review.json --require-ready --format json > push-readiness.json
