...
- live workflow-status collection depends on a functioning authenticated GitHub CLI, requires every admitted run to report the exact requested commit as its head SHA, applies a 15-second timeout to each live `git`/`gh` subprocess, and refuses a result set that exceeds the configured 20-run review window rather than treating truncated workflow evidence as complete; it still does not rerun workflows or prove that the configured checks are sufficient;
...
Latest stewardship run: **AUTO-206 — bind live workflow rows to the requested commit inside the shared collector**.

- **Changed:** the existing bounded GitHub workflow-status collector now requires every admitted `gh run list --commit <sha>` row to contain a non-empty `headSha` exactly equal to the requested commit. Missing or mismatched run SHAs fail closed before the payload can become commit-status review evidence. Successful payloads record `workflow_run_commit_binding_complete=true`.
- **Why:** AUTO-203 enforced this binding inside `verified-push-run`, but the shared collector itself still returned rows without proving their commit identity. Direct `commit-status-review --from-github` and any future collector consumer could therefore rely on a top-level requested SHA while admitting an inconsistent workflow row. AUTO-206 closes that shared evidence-boundary defect instead of duplicating consumer-specific checks.
- **Validation:** the pre-run AUTO-205 head `88d806df43afa583f5b2f82a26aee3a12a1e7d8c` is green in Actions run 32864546711. AUTO-206 adds deterministic coverage for successful collector-level binding, missing-head refusal, mismatched-head refusal, and direct live CLI compatibility. The substantive implementation/test Actions run is inspected before completion is claimed.
- **Safety:** this hardens the already-approved live-status path and adds no new external command type, network surface, workflow rerun, push authority, force-push/tag-push behavior, or remote/protection mutation. The existing 15-second command timeout and 20-run-plus-sentinel completeness gate remain unchanged.
...
- **Visual updates:** none; this strengthens commit identity at an existing evidence boundary without changing lifecycle topology.
- **Current limitations:** commit trust and branch protection remain caller-supplied; live status still depends on installed/authenticated `gh`; the collector proves returned workflow rows belong to the requested commit but does not prove the repository's configured checks are sufficient.
- **Next autonomous objective:** inspect AUTO-206 CI first; if green, continue the integrated guarded-maintenance milestone with the next concrete cross-stage integrity defect or policy-permitted evidence-handoff reduction rather than adding another isolated read-only command.