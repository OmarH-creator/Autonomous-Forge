...
Latest stewardship run: **AUTO-191 — refuse stale guarded-patch apply and rollback writes**.

- **Changed:** guarded patch publication now re-reads the target immediately before `os.replace` and requires it to match the exact text that authorized the write. The initial apply expects the preview-matched original bytes; rollback expects Forge's just-applied replacement bytes.
- **Why:** AUTO-190 made replacement crash-safe, but a concurrent edit after preview preparation could still be silently overwritten during apply, and a third-party edit made after Forge's apply could be overwritten by automatic rollback after live-diff verification failed.
- **Safety:** stale target state now blocks before replacement and preserves the newer bytes. Rollback also fails closed rather than clobbering a concurrent edit. Existing preview reproduction, explicit `--confirm-apply`, path/symlink containment, secret-marker checks, atomic/fsynced replacement, live-diff verification, validation, commit, and push gates remain unchanged.
...
- **Validation:** deterministic AUTO-191 regression tests cover stale apply refusal and failed-live-diff rollback preserving a third-party edit. Direct full checkout/pytest remains unavailable because this runtime cannot resolve `github.com`; the connected GitHub status surface currently exposes no checks for the new head, so no unsupported green Python 3.10/3.11/3.12 claim is made.
- **Visual updates:** none; the maintenance lifecycle is unchanged and the existing Mermaid flow remains accurate.
- **Current limitations:** this is a fail-closed stale-state recheck rather than a shared filesystem lock or atomic compare-and-swap; a writer can still race in the narrow interval between the final comparison and `os.replace`.
- **Next autonomous objective:** inspect AUTO-191 CI when observable; any failure takes priority. If green, continue only with another concrete mutation/persistence integrity defect or a meaningful reduction in caller-managed evidence handoffs.
