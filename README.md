...
After all retained validation commands pass, verified change orchestration hashes the exact target bytes into commit-readiness evidence. Verified commit creation re-hashes that target immediately before staging and refuses the commit if the file changed after validation. After staging, Forge also hashes the exact index bytes for the target and requires that staged SHA-256 to match the validated target digest before `git commit` can run. Forge now also binds the commit to the exact reviewed parent `HEAD`: it captures that parent before staging, rechecks `HEAD` immediately before commit creation, and refuses to commit if the branch moved. After Git creates the commit, Forge verifies both the target bytes and the created commit parent before the commit can be marked verified.
...
- validation-to-commit target hashing verifies working-tree bytes before staging, exact staged index bytes afterward, the reviewed parent `HEAD` before and immediately before commit creation, the exact parent of the created commit, and the exact target bytes stored in the created commit. These are fail-closed checks rather than a shared Git index/ref lock, so an extremely narrow concurrent mutation may still create a commit; such a commit is detected and reported as `created_unverified` rather than silently accepted;
...
Latest stewardship run: **AUTO-195 — bind verified commits to the reviewed parent HEAD**.

- **Changed:** verified commit creation now captures the exact parent `HEAD` after reviewed-path status inspection and before staging, rechecks `HEAD` after staged-byte verification immediately before `git commit`, and refuses commit creation if the branch moved. After commit creation it also resolves `<created_sha>^` and requires that parent to equal the reviewed parent before the commit can be marked verified. The report retains `reviewed_parent_commit`, `precommit_parent_commit`, and `created_commit_parent`.
- **Why:** AUTO-194 proved target-byte continuity through the created commit, but a concurrent process could still advance or rewind the branch so that an otherwise byte-correct commit was created on an unreviewed base.
- **Safety:** ordinary parent drift now blocks before `git commit`; a narrower race that still creates a commit on the wrong parent becomes `created_unverified`. Forge does not reset or rewrite that commit automatically. Existing target hashing, reviewed-path staging, exact changed-path verification, commit confirmation, hooks, and all push/remotes protections remain unchanged.
...
- **Validation:** deterministic regression coverage was added for pre-commit parent drift blocking and post-commit wrong-parent detection. Existing verified-commit tests remain compatible with the unchanged SHA/summary `git show` contract. Direct clone/full pytest remains unavailable because this runtime cannot resolve `github.com`; the Python 3.10/3.11/3.12 matrix is not claimed green without observable CI evidence.
- **Visual updates:** none; this is an integrity guard inside the existing commit-verification stage, so the lifecycle diagram remains accurate.
- **Current limitations:** the checks reduce and detect parent drift but are not a shared Git ref/index lock or compare-and-swap commit primitive. A created-but-unverified commit still requires human inspection.
- **Next autonomous objective:** inspect AUTO-195 CI when observable; any failure takes priority. If green, continue only with another concrete cross-stage integrity defect or a meaningful caller-managed evidence-handoff reduction.
