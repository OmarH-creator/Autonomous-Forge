...
The project starts as a local-first Python CLI. Its first goal is deliberately small: provide a `forge` command that can grow into dry-run planning, task selection, validation reporting, diff review, supplied commit-status review, combined change readiness, guarded patch preview, explicitly confirmed patch application, post-apply validation handoff, commit-readiness review, commit metadata preview, explicitly confirmed local commit creation, post-commit verification, push-readiness review, an explicitly confirmed non-force push handoff, post-push verification, and durable repository memory without requiring uncontrolled autonomous behavior.

For a visual orientation to the current workflow and its safety boundary, see [the project overview](docs/OVERVIEW.md).

## Current Autonomous Status

Autonomous Forge is pre-alpha. Latest autonomous run: AUTO-098 added `forge post-push-verify` and compatibility `forge-post-push-verify`, a post-push verification gate that consumes pushed `forge push-handoff --format json` evidence and clear `forge commit-status-review --format json` evidence, then confirms the pushed commit is reachable from the intended remote branch using local remote-tracking refs. It supports an explicit bounded `--fetch`, validates safe reviewed paths, branch, and remote labels, reports whether the commit is the remote branch head or reachable from it, and keeps all push/force-push/commit/remote/protection mutation outside the command. Direct full checkout/test execution remains unavailable in this environment; validation included scratch syntax compilation plus focused pytest for the new post-push verification tests, static GitHub API review, and CI smoke coverage for the new primary and compatibility help routes. No visual updates were needed because the existing overview still accurately describes the safety boundary. Current limitations: no signed commit verification, cryptographic trust model, automatic validation execution after patch application, or remote workflow rerun/polling. Next autonomous objective: add a durable end-to-end release/maintenance evidence bundle that links patch apply, validation, commit, push, and post-push verification reports.
...
- Smoke and deterministic coverage for the CLI’s current local workflows, including primary and compatibility post-push verification behavior.
