...
After all retained validation commands pass, verified change orchestration now hashes the exact target bytes into commit-readiness evidence. Verified commit creation re-hashes that target immediately before staging and refuses the commit if the file changed after validation, preventing stale or newly edited bytes from entering a supposedly validated commit.
...
- validation-to-commit target hashing closes ordinary post-validation drift but is not a shared lock, so a writer can still race between the final SHA-256 check and `git add`;
...
Latest stewardship run: **AUTO-192 — bind validated target bytes through commit creation**.

- **Changed:** after every retained validation command passes, verified change orchestration hashes the exact reviewed target bytes and carries that SHA-256 into verified commit-readiness evidence. Verified commit creation re-hashes the target immediately before any Git status/staging operation and refuses to stage if the bytes no longer match.
- **Why:** the prior commit boundary proved validation completion, reviewed paths, and post-commit changed paths, but a file edited after validation and before `git add` could still enter the commit even though those exact bytes had never passed validation.
- **Safety:** post-validation drift now blocks before Git is invoked. Target hashing is bounded to 1 MB, rejects symlinks and repository-root escapes, and does not add any commit, push, remote, network, workflow, force-push, tag-push, or protection authority. Existing separate commit confirmation and post-commit exact-path verification remain unchanged.
...
- **Validation:** deterministic AUTO-192 regression coverage now checks target SHA-256 capture, change-run propagation, and post-validation target drift refusal before any Git runner call. Direct full checkout/pytest remains unavailable because this runtime cannot resolve `github.com`; the connected GitHub status surface currently exposes no checks for the new code/test head, so no unsupported green Python 3.10/3.11/3.12 claim is made.
- **Visual updates:** none; the maintenance lifecycle is unchanged and the existing Mermaid flow remains accurate.
- **Current limitations:** the byte binding is a fail-closed staleness check rather than a filesystem-level lock. A concurrent writer can still race in the narrow interval after the final SHA-256 check and before `git add`.
- **Next autonomous objective:** inspect AUTO-192 CI when observable; any failure takes priority. If green, continue only with another concrete cross-stage mutation/provenance integrity defect or a meaningful reduction in caller-managed evidence handoffs.
