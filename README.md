...
- `forge verified-maintenance-run`: post-push-verified evidence → canonical durable bundle → `.ai/run-history/` link, with separate persistence confirmations. Durable verified provenance now retains the same normalized live workflow-status proof and a deterministic SHA-256 of that proof when live status was used.
...
Latest stewardship run: **AUTO-210 — preserve live-status provenance in durable maintenance evidence**.

- **Changed:** durable `verified_provenance` now retains normalized `live_status_evidence` already validated by push readiness: exact requested commit, bounded workflow-run limit, completeness proof, per-run commit-binding proof, and a deterministic SHA-256 of the normalized proof.
- **Why:** AUTO-209 made the proof directly reviewable on `verified-push-run`, but durable maintenance consumers still had to reopen nested push evidence. AUTO-210 carries the same field across the durable boundary without inventing a parallel status-evidence contract.
- **Validation:** AUTO-209 final head `76f15cc56f222702ca473a01dd4d7aef657ea5f4` is green in Actions run 32941261985. AUTO-210 adds deterministic coverage for successful durable preservation/hashing, wrong-commit blocking, and backward-compatible supplied non-live status; final-head CI is inspected before completion is claimed.
- **Safety:** malformed, incomplete, over-limit, or wrong-commit live proof blocks durable provenance. No network/external-command surface, workflow rerun, push authority, force-push/tag-push behavior, or remote/protection mutation was added.
...
- **Visual updates:** none; this extends evidence continuity through an existing durable stage without changing lifecycle topology.
- **Current limitations:** commit trust and branch protection remain caller-supplied; live status still depends on installed/authenticated `gh`; the small maintenance history link does not yet carry its own compact hash-bound live-status summary.
- **Next autonomous objective:** inspect AUTO-210 CI first; any failure takes priority. If green, derive a compact hash-bound live-status summary for the maintenance history link from this durable field, without creating parallel semantics.
