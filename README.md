...
Latest stewardship run: **AUTO-211 — preserve live-status provenance in maintenance history links**.

- **Changed:** completed maintenance history links now retain a compact `live_status_evidence_summary` when their written durable bundle contains normalized live workflow-status provenance. The summary carries the exact requested commit, bounded workflow-run limit, completeness proof, per-run commit-binding proof, and the established deterministic evidence SHA-256.
- **Why:** AUTO-210 made live-status proof durable in the full maintenance bundle, but compact run-history consumers still had to reopen that bundle merely to discover the proof. AUTO-211 carries the same contract across the history-link boundary without creating parallel workflow-status semantics.
- **Validation:** pre-run AUTO-210 head `000200093e5372b1dafd1820f84295291f88d80b` is green in Actions run `32962092448`. AUTO-211 adds deterministic coverage for successful summary persistence/text exposure, digest-tamper blocking, wrong-commit blocking, and legacy/non-live compatibility; final-head CI is inspected before DONE is claimed.
- **Safety:** the writer recomputes the normalized live-status SHA-256 and revalidates source, exact bundle commit, run limit, completeness, and per-run binding before history-link persistence. No network/subprocess surface, workflow rerun, push authority, force-push/tag-push behavior, or remote/protection mutation was added.
...
- **Visual updates:** none; this extends evidence continuity through the existing durable-history stage without changing lifecycle topology.
- **Current limitations:** commit trust and branch protection remain caller-supplied; live status still depends on installed/authenticated `gh`; the history-link summary is compact discoverability evidence and the linked durable bundle remains authoritative.
- **Next autonomous objective:** inspect AUTO-211 CI first; any failure takes priority. If green, extend maintenance history-link review so `--verify-linked-bundle` verifies and exposes this same compact live-status summary against the linked durable bundle.
