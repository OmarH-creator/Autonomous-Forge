...
The project starts as a local-first Python CLI. Its first goal is deliberately small: provide a `forge` command that can grow into dry-run planning, task selection, validation reporting, diff review, supplied commit-status review, combined change readiness, guarded patch preview, explicitly confirmed patch application, post-apply validation handoff, and durable repository memory without requiring uncontrolled autonomous behavior.

For a visual orientation to the current workflow and its safety boundary, see [the project overview](docs/OVERVIEW.md).

## Current Autonomous Status

Autonomous Forge is pre-alpha. Latest autonomous run: AUTO-090 added `forge post-apply-validation` and compatibility `forge-post-apply-validation`, a read-only handoff that consumes a successful `forge patch-apply` JSON report plus explicit supplied validation metadata and reports whether the applied change has passed every required validation step. This closes the immediate post-apply gap without running commands, polling workflows, writing files, committing, or pushing. Direct local checkout/test execution remains unavailable in this environment, so validation was limited to static GitHub API source/test/docs review plus committed deterministic tests. No visual updates were needed because the existing overview still accurately describes the local-first safety boundary. Current limitations: no live workflow polling, cryptographic commit verification, automatic validation execution after patch application, or commit-readiness command. Next autonomous objective: add a separate commit-readiness summary that consumes post-apply validation plus final diff/status evidence before any commit workflow is considered.
