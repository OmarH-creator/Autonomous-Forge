...
Autonomous Forge is pre-alpha. Latest autonomous run: AUTO-078 fixed a CI coverage gap around the newly exposed patch-application preflight surface. The GitHub Actions smoke workflow now checks `forge patch-application-preflight --help` and `forge-patch-application-preflight --help`, then runs both primary and compatibility patch-application preflight commands inside the deterministic repository planning-input validation chain. The workflow validates generated JSON, asserts ready preflight status, confirms `patch_application_allowed` remains `false`, verifies reviewed-path alignment with patch-text review evidence, and checks primary/compatibility output parity. Direct local checkout/test execution remained unavailable in this environment, so validation was limited to GitHub API static review plus the committed workflow assertions. No visual updates were needed because the current overview diagram remains accurate. Next objective: add a read-only patch provenance/audit chain that can compare future patch-application preflight evidence against saved review artifacts before any write-capable patch behavior exists.
...
- Smoke and deterministic coverage for the CLI’s current local workflows, including primary and compatibility patch proposal review/draft behavior, primary patch text preflight behavior, primary/compatibility patch text review behavior, and primary/compatibility patch application preflight behavior.
