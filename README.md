...
- immutable validation sidecars are visible through `run-history-read`, replay, durable bundle construction, maintenance history-link summaries, and linked-bundle history review, but remain externally supplied observations and are never promoted to executor-produced validation evidence;
...
Latest stewardship run: **AUTO-174 — verify compact external validation provenance during linked history replay**.

- **Changed:** `forge maintenance-history-link-review --verify-linked-bundle` now recomputes SHA-256 for the linked bundle's full `external_validation_evidence` block and compares it with a present history-link `external_validation_evidence_summary`. Source record, attachment count, and fixed advisory/non-executor semantics are checked at the same boundary.
- **Safety:** a present-but-tampered summary blocks linked replay and makes `--require-linked-replayable` fail. External observations remain `externally_supplied_observation`, `executor_validation_equivalent: false`, and `bundle_gate_effect: advisory_only`; the verifier never promotes them to executor proof. Legacy links without the compact summary remain compatible and report `status=not_present` rather than being rejected solely for age.
...
- **Validation:** deterministic AUTO-174 coverage checks verified summary/hash continuity, tampered-hash refusal, attempted executor-proof promotion refusal, and legacy-link compatibility. Direct full-checkout pytest remains unavailable because this runtime cannot resolve `github.com`; final GitHub status/workflow evidence is inspected separately and no green matrix claim is made without observable evidence.
- **Visual updates:** none; this strengthens the existing history-link → linked-bundle verification edge and does not change the lifecycle architecture.
- **Current limitations:** compact SHA-256 verification proves consistency with the linked bundle bytes, not signer identity. A legacy link may reference a bundle containing external advisory evidence without carrying the newer compact summary; that absence remains explicitly visible rather than blocking replay.
- **Next autonomous objective:** inspect AUTO-174 CI when observable; if green, expose the same verified advisory-provenance result through the maintenance review handoff/comparison surfaces so reviewers can preserve it without reopening low-level linked replay JSON.
