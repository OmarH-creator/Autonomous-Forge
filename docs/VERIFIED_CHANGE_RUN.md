...
After `git add`, Forge reads the exact staged target from the index with `git show :<target>`, applies the same 1 MB bound, computes its SHA-256, and requires it to equal the validated target digest before `git commit` is allowed. It also inspects the complete staged path set with NUL-delimited `git diff --cached --name-only -z --` and requires that set to exactly equal the reviewed paths. This prevents unrelated files that were already staged, or that became staged during the commit-preparation window, from being included in the commit. The staged target digest and staged path list are retained in the verified commit-creation report for review.
