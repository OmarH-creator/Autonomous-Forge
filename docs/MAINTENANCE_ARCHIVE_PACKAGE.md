...
Forge compares the byte count and SHA-256 of every source entry against the already-reviewed package preview while those exact bytes are being written. If a source changes after preview but before or during packaging, construction fails closed and the temporary package is discarded rather than publishing an artifact that no longer matches the reviewed evidence.

This bounds peak memory for large evidence while strengthening the preview-to-write integrity handoff. Exact packaging and hashing still read every source byte, so runtime and I/O remain proportional to archive size.
...
- `2`: inputs were invalid, missing, unsafe, unready, package creation was not explicitly confirmed, source evidence changed during packaging, package construction failed, or race-safe publication could not complete.
