...
The command accepts repository-relative paths only. Absolute paths, parent-directory traversal, backslash-separated paths, and paths that resolve outside `--root` through symbolic links are reported as `unknown` and are never checked for filesystem presence. The summary reports total reviewed paths and counts by policy status.
