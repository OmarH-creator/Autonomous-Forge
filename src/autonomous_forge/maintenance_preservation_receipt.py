...
def _sha256_file(path: Path) -> str:
    """Hash one published receipt in bounded chunks for ownership checks."""
...
def _rollback_owned_receipt(output: Path, *, expected_sha256: str) -> None:
    """Remove a failed receipt publication only while its bytes still belong to this invocation."""
    try:
...
def write_maintenance_preservation_receipt(completeness_path: Path, output_path: Path, *, root: Path = Path("."), confirm_write: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            _rollback_owned_receipt(output, expected_sha256=expected_sha256)
            raise
        ...
        if published:
            try:
                ...
                raise MaintenancePreservationReceiptError(
                    f"receipt persistence failed: {exc}; ownership-checked rollback failed: {rollback_exc}"
                ) from exc
