"""Summarize push readiness from verified commit and fresh status evidence."""
...
_SAFE_BOUNDARY = (
    "Push-readiness reads supplied commit-verify and commit-status-review JSON evidence only. "
    "It never runs git, calls networks, stages files, creates commits, pushes, changes remotes, "
    "reads environment variables, or modifies the working tree."
)
...
def build_push_readiness_data(
    ...
    """Build deterministic push-readiness data from verified commit and status evidence."""
    ...
    return {
        ...
        "source": "supplied commit-verify and commit-status-review JSON evidence",
        ...
        "next_step": (
            "Use this ready report for human review before designing any separately confirmed push command."
            if readiness_status == "ready"
            else "Resolve commit verification or workflow-status blockers before considering any push workflow."
        ),
...
def read_push_readiness(
    ...
    *,
    ...
    return build_push_readiness_data(commit_verify, status_review)
