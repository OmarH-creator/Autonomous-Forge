...
def build_git_diff_review_data(policy_text: str, diff_text: str, *, root: Path = Path(".")) -> dict[str, Any]:
    ...
    requires_attention = bool(parse_warnings or not file_changes or prohibited or unknown)
    return {
        ...
        "reason": (
            "Review prohibited, unknown, empty, or malformed diff evidence before implementation continues."
            if requires_attention
            else "All diff paths match documented allowed policy paths and the supplied diff parsed cleanly."
        ),
        "next_step": (
            "Resolve diff review blockers or request human approval before patch application."
            if requires_attention
            else "Use this clear diff review as advisory input for human review and future guarded patch application."
        ),
