...
### AUTO-046 through AUTO-048 — Narrow executor, result handoff, and guarded handoff persistence
Priority: P1
Status: DONE

Goal: Execute one exact executor-contract candidate locally and provide a guarded bridge from observed executor output to durable local history.
Why it matters: The product needs to move from review-only validation planning toward a controlled end-to-end maintenance workflow, while preserving strict command allowlists, timeouts, reviewable result capture, and explicit persistence.
Scope: Added `forge executor-run --format text|json`, a no-shell local validation executor that accepts only exact gated validation commands, reports completed/timed-out/launch-failed observations, emits an advisory `persistence_handoff`, and now includes a package-level helper that consumes reviewed executor-run JSON and writes through the validation-result writer only after explicit confirmation.
Expected files or areas: `src/autonomous_forge/`, `tests/`, README, `docs/`, `.github/workflows/test.yml`, `.ai` records.
Acceptance criteria: The executor only runs allowlisted local validation commands after `--confirm-executor-dry-run`, never uses a shell, never mutates files directly, reports observed exit status or launch failure, exposes persistence handoff data, rejects unavailable or mismatched handoffs, preserves failed results, and writes only one guarded `.ai/run-history/*.json` record through validation-result writer semantics after confirmation.
Validation: Static review completed through the GitHub repository API. Deterministic tests cover blocked/unconfirmed execution, exact candidate execution with a fake no-shell runner, failed return-code mapping, launch-failure mapping, unknown/shell command blocking, handoff command construction, unavailable handoff refusal, mismatched result refusal, unsafe record path refusal, and confirmed handoff persistence. CI smoke coverage exercises the installed executor-run path; direct local pytest execution remained unavailable in this environment.
...
Notes: The persistence helper is intentionally package-level first; the next safe step is a narrow CLI wrapper with the same guards.

### AUTO-049 — Expose guarded executor-handoff persistence through CLI
Priority: P1
Status: TODO

Goal: Add a narrow CLI command that consumes reviewed executor-run JSON and persists its handoff through existing validation-result writer semantics.
Why it matters: Maintainers need a usable command-line bridge from executor output to durable history without copying fields by hand.
Scope: Add CLI parser/dispatch for one executor-handoff persistence command, text/JSON summaries, deterministic tests, and CI smoke coverage where practical.
Expected files or areas: `src/autonomous_forge/cli.py`, `tests/`, README, `docs/`, `.ai` records.
Acceptance criteria: The command rejects malformed/missing executor output, refuses unavailable or mismatched handoffs, requires `--confirm-write`, preserves failed results, writes only one guarded `.ai/run-history/*.json` record, and never reruns validation.
Validation: Add deterministic CLI tests and smoke coverage if the command is safe to run in CI.
Risks or assumptions: Do not combine validation execution and persistence into one implicit command.
Notes: This is the next safest step before any patch generation or diff-inspection behavior.
