...
The immediate roadmap is baseline recovery under issue #13: `main` remains red, so new feature work is paused. AUTO-141 repaired the importable primary router's successful help-return contract without swallowing parser failures. AUTO-142 continues the same P0 milestone by correcting replay-context compatibility, making exact pytest failure identities visible without suppressing CI failures, aligning stale archive fixtures with generated evidence, repairing maintenance-review handoff context comparison, and now canonicalizing absolute-but-repository-contained archive entry paths before copy-preview and copy-verification destination joins. That archive-path repair removed the entire observed archive-copy/verify/package/preservation failure cluster and reduced the inspected Python 3.11 suite from 57 failed / 597 passed at cycle start to 23 failed / 631 passed. Remaining work is limited to separate stale enriched-output assertions, one replay-policy router-help assertion, and maintenance-review-compare fixture/output failures.
...
Why it matters: `main` remains red, and the largest historical failure group came from newer context-consistency semantics interacting with older partial evidence fixtures and path-shape assumptions. Feature delivery must remain paused until the baseline is green.
Scope: Correct proven compatibility defects in replay/context handling; ensure maintenance-review handoff compares raw retained history evidence with replayed bundle context rather than summary counts; canonicalize repository-contained archive entry paths before destination mapping; update deterministic fixtures or assertions to the current safe contract where appropriate; preserve fail-closed behavior for explicit mismatches and malformed evidence; continue the same milestone across remaining tests until the baseline is green; and keep CI diagnostics actionable enough to identify exact failing test node IDs without weakening the suite.
Expected files or areas: `src/autonomous_forge/maintenance_replay_summary.py`, `src/autonomous_forge/maintenance_review_handoff.py`, `src/autonomous_forge/maintenance_archive_copy_preview.py`, `src/autonomous_forge/maintenance_archive_copy_verify.py`, maintenance/archive/replay tests under `tests/`, `.github/workflows/test.yml`, README, `.ai` records, issue #13.
Acceptance criteria: Missing optional context is not treated as contradictory evidence; supplied expected-change context must still cover reviewed paths; handoff context consistency compares retained values rather than summary counts; retained validation-step drift still blocks; malformed or integrity-drifted evidence still blocks; archive sources inside the repository are canonicalized to repository-relative paths before being mapped below the selected archive root; source/destination containment and overwrite guards remain fail-closed; remaining contract failures are repaired without weakening safety; failed pytest runs expose actionable failing-test identities while preserving pytest's exit status; full Python 3.10/3.11/3.12 pytest matrix ultimately passes before feature work resumes.
Validation: The first AUTO-142 slice added a deterministic partial-context regression case and preserved existing explicit path-mismatch and validation-step-mismatch tests. GitHub Actions run `31841541663` passed install, compile, CLI smoke, and roadmap lint but failed at pytest across Python 3.10, 3.11, and 3.12. Commit `80efd9c4423cc7208891533a0c55ddc4ec0fb657` added failure-name annotations without changing pytest's exit status. Those annotations exposed the archive cluster. Fixture commits `f01a9c7f2d83f7d6c6a5673cfdaac926d0842713` and `9f30db304460e6f2f1466ad8efbdec8659e07eba` aligned source-report metadata and expected-change text with generated evidence. Commit `f03aed1c34285c041f02afa236898090ad8d9022` repaired the handoff summary/raw-context defect. In the current slice, `bedb09e77ab672e8716ab1fc3910a06549afd478` canonicalized archive-copy preview sources after repository containment and `8160132ad188033d66a6290c180bbd718fe42952` applied the same rule to copied-root verification. The inspected Python 3.11 matrix improved from 57 failed / 597 passed at cycle start to 23 failed / 631 passed, and no archive-copy, archive-copy-verify, archive-package, or preservation-completeness tests remain in the failure summary. All three Python jobs still pass install, compile, CLI smoke, and roadmap lint before pytest.
Risks or assumptions: Direct repository cloning is unavailable from this runtime because outbound DNS to github.com is blocked. The CI diagnostic wrapper relies on pytest's standard `FAILED ` summary lines and caps the annotation at 80 entries; if no such summary is produced, the job still fails with a generic diagnostic annotation. Compatibility changes must never suppress explicit mismatch, malformed-evidence, hash-integrity, parser-error, containment, overwrite, or side-effect safeguards. Remaining failures are separate from the resolved archive-path defect and should be repaired as distinct coherent slices.
Notes: Continue this same P0 milestone across subsequent autonomous runs until `main` is green or a concrete external blocker prevents further repair. Next prioritize stale enriched planning/validation/executor output-contract assertions, then the replay-policy help assertion and maintenance-review-compare fixture/output failures.
