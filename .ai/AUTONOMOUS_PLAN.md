...
Scope: Add content-audit, diff-source handoff, patch-intent review, patch-intent description, patch proposal manifest, patch proposal review, validation-step enforcement, primary/compatibility CI coverage, `forge patch-proposal-draft --format text|json` with `--require-draft-ready`, `forge-patch-proposal-draft` compatibility, and `forge patch-text-preflight --format text|json` with `--require-ready`.
