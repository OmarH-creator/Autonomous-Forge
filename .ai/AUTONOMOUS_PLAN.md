...
Validation: Static review completed through the GitHub repository API. Deterministic tests cover blocked/unconfirmed execution, exact candidate execution with a fake no-shell runner, failed return-code mapping, launch-failure mapping, unknown/shell command blocking, and CLI JSON refusal behavior. CI smoke coverage runs `forge executor-run --command "python -m pytest" --confirm-executor-dry-run --format json` in a checkout-capable environment. Direct local pytest execution remained unavailable in this environment.
...
Scope: Added a deterministic `persistence_handoff` to `forge executor-run` text/JSON output that exposes the exact `forge validation-result-write --confirm-write` command, result, note, target record, and no-auto-persistence boundary.
Expected files or areas: `src/autonomous_forge/executor_run.py`, `tests/test_executor_run.py`, README, `docs/EXECUTOR_RESULT_HANDOFFS.md`, `.ai` records.
Acceptance criteria: The handoff is deterministic, does not auto-write, preserves the observed return code/result/note, is unavailable for blocked runs, and documents the confirmation boundary.
Validation: Static review completed through the GitHub repository API. Deterministic tests cover blocked handoffs, successful handoff command construction, JSON fields, and text output. Direct local pytest execution remained unavailable in this environment.
...
Notes: This continues the executor milestone rather than starting a documentation-only task.

### AUTO-048 — Guard reviewed executor handoff persistence
Priority: P1
Status: TODO

Goal: Add a narrow explicit persistence path that consumes reviewed executor handoff data while preserving `--confirm-write` and saved-history path guards.
Why it matters: Maintainers should be able to move from observed validation execution to durable history without copying fields by hand, but the tool must still avoid automatic or implicit mutation.
Scope: Add a helper that accepts executor-run JSON, verifies the handoff shape, refuses missing or blocked handoffs, and writes only through the existing validation-result writer after explicit confirmation.
Expected files or areas: `src/autonomous_forge/`, `tests/`, README, `docs/`, `.ai` records.
Acceptance criteria: The helper rejects malformed/missing executor output, refuses unavailable handoffs, requires `--confirm-write`, preserves failed results, writes only one guarded `.ai/run-history/*.json` record, and never reruns validation.
Validation: Add deterministic tests and CI smoke coverage where practical.
