...
Scope: Add `forge content-audit --format text|json`, audit explicit repository-relative paths for policy status, bounded UTF-8 metadata, configured secret-like markers, and conservative review status; keep limited history audits focused on newest filename-sorted records; add installed-package CI semantic assertions for live content-audit output; harden hidden expected-file path parsing; add `forge diff-source-handoff --format text|json` to compare two supplied content-audit JSON outputs; add `forge diff-source-handoff --require-clear`; add `forge patch-intent-review --format text|json` with `--require-ready`; add `forge patch-intent-describe --format text|json` with `--require-described`; refuse unsafe patch-intent description candidate path labels; add `forge patch-proposal-manifest --format text|json` with `--require-ready`; add patch proposal review with `--require-ready`; expose that gate as `forge patch-proposal-review` while preserving `forge-patch-proposal-review`; fail closed when a supplied proposal manifest lacks at least one non-empty validation step; and smoke-test primary and compatibility proposal-review routes in CI.
Expected files or areas: `pyproject.toml`, `src/autonomous_forge/cli_entry_patch.py`, `src/autonomous_forge/content_audit.py`, `src/autonomous_forge/diff_source_handoff.py`, `src/autonomous_forge/patch_intent_review.py`, `src/autonomous_forge/patch_intent_description.py`, `src/autonomous_forge/patch_proposal_manifest.py`, `src/autonomous_forge/patch_proposal_review.py`, `src/autonomous_forge/patch_proposal_review_cli.py`, `src/autonomous_forge/cli_entry.py`, `tests/`, `.github/workflows/test.yml`, README, `docs/`, `.ai` records.
Acceptance criteria: Patch proposal reviews read supplied manifest JSON plus supplied fresh content-audit JSON only, require exact requested/audited path support with clear review status, require at least one non-empty validation step, support `--require-ready`, are available through `forge patch-proposal-review`, preserve standalone compatibility, and never read repository file contents, inspect git diffs, generate patches, apply patches, run commands, or approve implementation. CI must exercise the documented primary route, the compatibility route, JSON validation, and equality for the same ready evidence.
Validation: Static review completed through the GitHub repository API. Deterministic unit/CLI tests cover the implemented content-audit, diff-source, patch-intent, patch-description, manifest, review, primary-routing, and compatibility behaviors. CI smoke assertions now cover unchanged live content-audit/diff-source/patch-intent/description/manifest/review evidence gates through `forge patch-proposal-review`, and separately verify the compatibility script emits identical JSON.
