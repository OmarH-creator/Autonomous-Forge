...
### AUTO-057 through AUTO-073 — Content audit and patch-proposal evidence readiness
Priority: P1
Status: DONE

Goal: Add read-only content-audit checkpoints, compare supplied content-audit outputs, provide fail-closed evidence gates, review patch-intent readiness, describe patch-intent handoff readiness, refuse unsafe candidate path labels, build explicit patch proposal manifests, review those manifests against fresh content-audit evidence, expose that final review through the primary `forge` command surface, require validation-step evidence, protect primary and compatibility proposal-review routes in CI, and preview patch proposal draft outlines from ready review evidence.
Why it matters: Patch-adjacent workflows need a safe bridge between policy-reviewed paths, content metadata, patch-intent readiness, safe candidate path labels, explicit objectives, requested paths, validation steps, fresh content-audit evidence, ready proposal reviews, and future patch proposal drafts without printing content, generating patches, inspecting git diffs, or treating changed evidence as safe by default.
Scope: Add content-audit, diff-source handoff, patch-intent review, patch-intent description, patch proposal manifest, patch proposal review, validation-step enforcement, primary/compatibility CI coverage, and `forge patch-proposal-draft --format text|json` with `--require-draft-ready` plus `forge-patch-proposal-draft` compatibility.
Expected files or areas: `pyproject.toml`, `src/autonomous_forge/cli_entry_patch.py`, `src/autonomous_forge/content_audit.py`, `src/autonomous_forge/diff_source_handoff.py`, `src/autonomous_forge/patch_intent_review.py`, `src/autonomous_forge/patch_intent_description.py`, `src/autonomous_forge/patch_proposal_manifest.py`, `src/autonomous_forge/patch_proposal_review.py`, `src/autonomous_forge/patch_proposal_draft.py`, `src/autonomous_forge/*_cli.py`, `tests/`, `.github/workflows/test.yml`, README, `docs/`, `.ai` records.
Acceptance criteria: Patch proposal draft previews read supplied proposal-review JSON only, require ready review status, require proposal allowance, require safe requested paths, require non-empty validation steps, support `--require-draft-ready`, are available through `forge patch-proposal-draft`, preserve standalone compatibility, and never read repository file contents, inspect git diffs, generate patches, apply patches, run commands, approve implementation, commit, push, or change files.
Validation: Static review completed through the GitHub repository API. Deterministic unit/CLI tests cover the implemented content-audit, diff-source, patch-intent, patch-description, manifest, review, draft, primary-routing, and compatibility behaviors. CI smoke assertions cover unchanged live content-audit/diff-source/patch-intent/description/manifest/review/draft evidence gates through primary and compatibility routes.
Risks or assumptions: Secret-marker checks are intentionally simple guard signals, not complete secret scanning. A clear content-audit, unchanged diff-source handoff, ready patch-intent review, described patch-intent description, ready patch proposal manifest, ready patch proposal review, or draft-ready patch proposal draft result does not approve patches, prove correctness, or replace human review. `--require-clear`, `--require-ready`, `--require-described`, and `--require-draft-ready` change only the process exit code.
Notes: The next safe step is adding a read-only patch text preflight gate that consumes draft-ready evidence plus explicit patch metadata without generating or applying patches.
...
- Read-only patch text preflight gates from draft-ready evidence.
