...
Scope: Reuse `validation-result-preview` data, require `--confirm-write`, constrain the target to `.ai/run-history/*.json`, preserve the existing schema, and write only the requested validation fields.
Expected files or areas: `src/autonomous_forge/`, `tests/`, README, `docs/`, `.ai` records.
Acceptance criteria: The command refuses missing confirmation, unsafe paths, malformed records, unsupported result values, and any write outside `.ai/run-history/`; it does not run validation commands or infer success.
...
Notes: This should remain narrower than a validation executor or workflow-status reader.
