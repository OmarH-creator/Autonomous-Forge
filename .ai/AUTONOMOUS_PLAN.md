...
### AUTO-059 — Harden planned file-area parsing for hidden policy paths
Priority: P1
Status: DONE

Goal: Resolve the concrete hidden-dotfile parsing blocker reported in PR #7 so roadmap tokens such as ``.env`.`` and ``.env`,`` are normalized as `.env` in change-proposal planned areas.
Why it matters: Policy-aware planning and proposal output depend on exact path tokens; a dangling backtick can hide or distort prohibited dotfile paths during human review.
Scope: Harden `_split_expected_areas` token cleanup and add deterministic regression coverage for sentence punctuation and trailing comma cases.
Expected files or areas: `src/autonomous_forge/proposal.py`, `tests/test_dotfile_paths.py`, README, `.ai` records.
Acceptance criteria: Hidden file names retain their leading period, surrounding backticks are peeled even when punctuation follows, trailing commas are ignored, and proposal behavior remains read-only.
Validation: Static review completed through the GitHub repository API. Regression tests were added for the existing hidden path case and the trailing comma case. Direct local pytest execution remained unavailable in this environment.
Risks or assumptions: The change intentionally stays limited to expected-area token normalization and does not broaden policy enforcement, parse multi-word grammar, inspect diffs, generate patches, or execute implementation plans.
Notes: This integrates the useful portion of open PR #7 directly onto `main`; the PR can be closed as obsolete after the mainline fix is recorded.
