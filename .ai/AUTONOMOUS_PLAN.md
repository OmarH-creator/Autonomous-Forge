...
The immediate roadmap is baseline recovery under issue #13: `main` remains red, so new feature work is paused. AUTO-141 repaired the importable primary router's successful help-return contract without swallowing parser failures. AUTO-142 continues the same P0 milestone by correcting replay-context compatibility, making exact pytest failure identities visible without suppressing CI failures, aligning stale archive fixtures with generated evidence, repairing maintenance-review handoff context comparison, canonicalizing repository-contained archive paths, and now collapsing semantically duplicate validation steps at the shared validation-plan boundary. The inspected Python 3.11 suite has improved from 57 failed / 597 passed earlier in the milestone, to 23 failed / 631 passed after archive recovery, and now to 22 failed / 633 passed after validation-step normalization. Remaining work is limited to stale enriched planning/validation/executor assertions, one replay-policy router-help assertion, and maintenance-review-compare fixture/output failures.
...
Why it matters: `main` remains red. Feature delivery must remain paused until the baseline is green, and the remaining failures are now narrow enough to repair as coherent compatibility/test-contract slices.
Scope: Correct proven compatibility defects in replay/context handling; compare retained raw history context correctly; canonicalize repository-contained archive paths; deduplicate semantically equivalent validation steps at the shared planning boundary; update deterministic fixtures or assertions to the current safe enriched contract where appropriate; preserve fail-closed behavior for explicit mismatches and malformed evidence; and continue the same milestone until the full matrix is green.
Expected files or areas: `src/autonomous_forge/validation.py`, maintenance/replay/archive modules already touched by AUTO-142, affected tests under `tests/`, README, `.ai` records, issue #13.
Acceptance criteria: Missing optional context is not treated as contradictory evidence; supplied mismatches still block; retained validation-step drift still blocks; malformed or integrity-drifted evidence still blocks; repository/archive containment and overwrite guards remain fail-closed; semantically duplicate validation steps do not create duplicate downstream command candidates; remaining contract failures are repaired without weakening safety; Python 3.10/3.11/3.12 pytest ultimately passes before feature work resumes.
Validation: The milestone has progressed through replay-context, CI-diagnostic, fixture, handoff-context, archive-path, and validation-step slices. Commit `d9a51b68089eec7926ce6d5310e232b61f6c54cb` added shared semantic validation-step deduplication and commit `43a74adf533239a1c52b5509bd10e25f97be8865` added focused regression coverage. GitHub Actions run `31861022809` passed install, compile, CLI smoke, and roadmap lint on Python 3.10, 3.11, and 3.12; the inspected Python 3.11 pytest result improved from 631 passed / 23 failed to 633 passed / 22 failed. Earlier archive recovery had already removed the archive-copy/verify/package/preservation cluster.
Risks or assumptions: Direct repository cloning is unavailable from this runtime because outbound DNS to github.com is blocked. Compatibility changes must never suppress explicit mismatch, malformed-evidence, hash-integrity, parser-error, containment, overwrite, or side-effect safeguards. The remaining failures are stale enriched planning/validation/executor assertions, one replay-policy router-help assertion, and four maintenance-review-compare fixture/output failures.
Notes: Continue this same P0 milestone across subsequent autonomous runs until `main` is green or a concrete external blocker prevents further repair. Next update stale enriched planning/validation/executor assertions to the current safe contract, then repair replay-policy help and maintenance-review-compare failures.
