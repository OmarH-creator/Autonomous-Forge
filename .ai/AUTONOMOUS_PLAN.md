...
Notes: Completed before combined change readiness.

### AUTO-086 — Add combined change-readiness summary
Priority: P1
Status: DONE

Goal: Combine clear supplied git-diff review and clear supplied commit-status review evidence into one guarded change-readiness summary before any future patch-applier workflow relies on a reviewed change.
Why it matters: Separate diff and status gates still force maintainers to manually connect changed paths to validation evidence. A combined summary gives the workflow a single reviewable checkpoint while keeping patch application disallowed.
Scope: Add `forge change-readiness` and compatibility `forge-change-readiness`, deterministic JSON/text summary logic, fail-closed `--require-ready`, tests, README usage, command documentation, CI smoke coverage, and project memory updates.
Expected files or areas: `src/autonomous_forge/change_readiness.py`, `src/autonomous_forge/change_readiness_cli.py`, `src/autonomous_forge/cli_entry_patch.py`, `pyproject.toml`, `tests/test_change_readiness.py`, `docs/CHANGE_READINESS.md`, `docs/COMMANDS.md`, `.github/workflows/test.yml`, README, and `.ai` records.
Acceptance criteria: The command accepts repository-local diff-review and status-review JSON files, refuses unsafe or out-of-root inputs, requires the upstream diff review to be clear, requires the upstream status review to be clear, reports reviewed paths and status contexts, supports `--require-ready`, keeps `change_application_allowed` false, and never calls networks, polls GitHub, runs workflows, runs commands, inspects raw diffs, reads repository file contents, generates patches, applies patches, approves implementation, commits, pushes, or changes files.
Validation: Static source/test/documentation/workflow review completed through the GitHub repository API. Deterministic tests were added for ready evidence, blocked diff/status evidence, JSON/text output, fail-closed ready gating, out-of-root refusal, and primary `forge` routing. CI smoke coverage was added for primary and compatibility installed routes. Direct local checkout/test execution remained unavailable in this environment.
Risks or assumptions: Supplied diff and status evidence can be stale, incomplete, or unrelated. This summary does not independently verify commits, poll live workflow status, prove correctness, generate patches, apply patches, or replace human review.
Notes: The next safe step is a guarded patch-generation preview design that can produce reviewable patch text only after ready change-readiness evidence, without automatic application.
...
- Guarded patch-generation preview after ready change-readiness evidence.
