...
- Current task ID: AUTO-034 — Add run-history comparison preview
...
- Known risks and assumptions: The new command compares only two explicit records under `.ai/run-history/`. It does not scan the history directory, verify commits, check workflow status, inspect diffs, read changed-file contents, run validation commands, generate patches, infer success, enforce policy, commit, push, call networks, or mutate files.
- Recommended next task: Add a guarded validation-result attachment preview before any validation executor, diff inspection, patch generation, index writer, or broader write behavior is considered.
